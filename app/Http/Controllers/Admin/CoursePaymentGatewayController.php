...
class CoursePaymentGatewayController extends Controller
{
    ...
    public function index()
    {
        ...
        // Seed Razorpay if not exists — credentials must be set (even empty)
        // so the NOT NULL constraint on the column is satisfied.
        if ($gateways->isEmpty()) {
            CoursePaymentGateway::create([
                ...
                'status'      => 0,
                'test_mode'   => 1,
                'credentials' => [          // ← mutator encrypts this; satisfies NOT NULL
                    'key_id'     => '',
                    'key_secret' => '',
                ],
            ]);
