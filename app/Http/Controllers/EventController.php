...
class EventController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $eventHero = [
            'eyebrow'         => $cms->hero_eyebrow   ?? 'Offline Events & Workshops',
            'title'           => $cms->hero_title      ?? 'Options Trading Events & Workshops',
            'title_highlight' => $cms->hero_title_highlight ?? 'Events',
            'subtitle'        => $cms->hero_subtitle
                ?? "Join India's top options traders, analysts & educators at CityQuants in-person events across the country.",
        ];

        $eventCta = [
            'title'     => $cms->cta_title     ?? null,
            'desc'      => $cms->cta_description ?? null,
            'btn_label' => $cms->cta_btn_label  ?? null,
            'btn_url'   => $cms->cta_btn_url    ?? '#',
        ];

        // Cities map for filter dropdown
        $citiesMap = $cms->cities_map;
    ...
    public function initiateBooking(Request $request, Event $event)
    {
        ...
        if (!$event->canBook()) {
            return response()->json(['success' => false, 'message' => 'Bookings are currently closed.'], 422);
        }
    ...
    public function verifyPayment(Request $request)
    {
        ...
        try {
            ...
            return response()->json(['success' => true, 'message' => 'Registration confirmed!']);
    ...
    private function confirmFree(Event $event, Request $request, ?int $userId)
    {
        ...
        return response()->json(['success' => true, 'message' => 'Registration confirmed! You will receive a confirmation email shortly.']);
    }
