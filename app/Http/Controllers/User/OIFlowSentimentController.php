...
/**
 * OI Flow Sentiment Analyzer — ALL logic lives here now (no service class).
 * T = today 14:45 | T-1 = prev day 15:00
 * CE↑+PE↓ → BULLISH  CE↓+PE↑ → BEARISH  (reversed logic, as currently live)
 *
 * NOTE: CpAnalysisSignalResolver (auto-order cron) still calls
 * OIFlowSentimentService::getSignalsForDate() separately. That service
 * file was NOT deleted, but it is now a duplicate of this logic, not a
 * shared source of truth. If you change the sentiment math here, you
 * must also change it there, or the cron and dashboard will drift.
 */
class OIFlowSentimentController extends Controller
{
    ...
    public function lastDate(Request $request): JsonResponse
    {
        try {
            ...
            Log::error('OIFlowSentimentController lastDate: ' . $e->getMessage());
    ...
    public function analyze(Request $request): JsonResponse
    {
        ...
        try {
            ...
            Log::error('OIFlowSentimentController analyze: ' . $e->getMessage());
            return response()->json(['success' => false, 'message' => $e->getMessage(), 'data' => []], 500);
        }
    }

    // ── Private helpers (ported from the service, unchanged) ────────────
