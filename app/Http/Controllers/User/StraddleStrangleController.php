...
/**
 * ╔══════════════════════════════════════════════════════════════════════════╗
 * ║              Straddle & Strangle — 1-Hour Candle Controller             ║
 * ╠══════════════════════════════════════════════════════════════════════════╣
 * ║  TWO MODES (mirrors 15-Min page behaviour):                              ║
 * ║                                                                          ║
 * ║  MODE 1 — symbol=ALL  (default)                                          ║
 * ║    One summary row per symbol — entry + latest candle only               ║
 * ║                                                                          ║
 * ║  MODE 2 — symbol=NIFTY (specific symbol selected)                        ║
 * ║    Every 1-hour interval row for that symbol                             ║
 ...
 * ╚══════════════════════════════════════════════════════════════════════════╝
 */
class StraddleStrangleController extends Controller
{
    ...
    {
        $pageTitle  = 'Straddle & Strangle (1-Hour)';
        $strategies = array_map(fn($s) => $s['name'], self::STRATEGIES);
        return view($this->activeTemplate . 'user.straddle-strangle.index',
            compact('pageTitle', 'strategies'));
    }

    // ══════════════════════════════════════════════════════════════════════════
    // Data API
    // GET /straddle-strangle/data?strategy=long_straddle&date=Y-m-d&symbol=ALL|NIFTY
    // ══════════════════════════════════════════════════════════════════════════
    ...
    public function getData(Request $request)
    {
        try {
            ...
            // Current time in minutes for candle completeness check (1hr candle = 60 min)
            ...
            // ── Load rows from DB ─────────────────────────────────────────────
            $allRows = ThirtyMinOhlcData::whereDate('trade_date', $today)
                ->whereIn('instrument_type', ['CE', 'PE'])
            ...
            if ($allRows->isEmpty()) {
                return $this->emptyJson($today,
                    "No 1-hour data for [{$def['name']}] on {$today}", $availableSymbols, $symbolFilter);
            }

            // ── Collect all unique trading symbols for lot size lookup ─────────
            ...
            foreach ($allRows as $row) {
                ...
                // Track latest COMPLETE candle (T + 60 min <= now for today)
                if ($isToday) {
            ...
            // Fallback: if market just opened and no complete candle yet
            ...
            if ($isAll) {
                ...
                return response()->json([
                    ...
                    'total'             => count($results),
                    'message'           => count($results) . ' symbols loaded for ' . $def['name'] . ' on ' . $today,
                ]);
            }

            // ══════════════════════════════════════════════════════════════════
            // MODE 2: Specific symbol — every 1-hour interval row
            // ══════════════════════════════════════════════════════════════════
            ...
            // Build one row per 1-hour interval
            $intervalRows = [];
            foreach ($times as $t) {
                $ce = $ceSlots[$t] ?? null;
                $pe = $peSlots[$t] ?? null;
            ...
            Log::error('StraddleStrangle getData: ' . $e->getMessage() . "\n" . $e->getTraceAsString());
    ...
    // ══════════════════════════════════════════════════════════════════════════
    // Resolve lot size — CE first, PE fallback, then 1
    // ══════════════════════════════════════════════════════════════════════════

    private function resolveLotSize(?string $ceTradingSymbol, ?string $peTradingSymbol, array $lotSizeMap): int
    {
    ...
    // ══════════════════════════════════════════════════════════════════════════
    // Build one summary row (MODE 1 — entry + latest only)
    // ══════════════════════════════════════════════════════════════════════════
    ...
    private function fetchOrCacheLiveLtp(array $symbols, array $def, array $indexed, string $date): array
    {
        ...
        foreach ($symbols as $sym) {
            foreach (['CE' => $def['ce_pos'], 'PE' => $def['pe_pos']] as $type => $pos) {
                $cacheKey  = "SS1H_LTP_{$date}_{$sym}_{$type}_{$pos}";
        ...
        if (!$broker) {
            Log::warning('SS1H: Broker [' . self::BROKER_CLIENT_ID . '] not found — using 09:15 open');
            return $result;
        }

        try {
            $helper = new BrokerZerodhaHelper($broker);
            // No preserve_keys — chunk values ARE the token integers
            ...
            foreach ($tokenChunks as $chunk) {
                ...
                try {
                    ...
                    Log::warning('SS1H LTP chunk failed: ' . $e->getMessage());
                }
            }
        } catch (\Exception $e) {
            Log::warning('SS1H broker init failed: ' . $e->getMessage());
        }
    ...
    public function clearLtpCache(Request $request)
    {
        ...
        $symbols = ThirtyMinOhlcData::whereDate('trade_date', $today)
            ->whereIn('instrument_type', ['CE', 'PE'])
        ...
        foreach ($symbols as $sym) {
            foreach (['CE' => $def['ce_pos'], 'PE' => $def['pe_pos']] as $type => $pos) {
                $k = "SS1H_LTP_{$today}_{$sym}_{$type}_{$pos}";
    ...
    private function calcExitDecision(?float $cePct, ?float $pePct): array
    {
        ...
        if ($cePct > 0 && $pePct > 0) {
            return ['EXIT BOTH', 'NONE',
                'Both CE OI (+' . $cePct . '%) & PE OI (+' . $pePct . '%) rising → Writers active both sides. Range market forming. EXIT BOTH legs.'];
        }
        return ['HOLD BOTH', 'BOTH',
            'Both CE OI (' . $cePct . '%) & PE OI (' . $pePct . '%) falling → Writers unwinding. Volatility expansion expected. HOLD BOTH — watch which OI builds first then exit opposite leg.'];
    }
