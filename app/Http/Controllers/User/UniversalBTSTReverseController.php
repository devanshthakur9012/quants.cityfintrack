...
/**
 * UniversalBTSTReverseController  — REVERSE / CONTRARIAN Logic
 *
 * Same as UniversalBTSTController but signal is FLIPPED:
 *   OI says BULLISH → BUY PE  (fade the signal)
 *   OI says BEARISH → BUY CE  (fade the signal)
 *
 ...
 * WIN    : profit_pct > +0.5%
 * LOSS   : profit_pct < -0.5%
 * NOISE  : |profit_pct| <= 0.5%  → ignored
 * FILTER : buyPrice >= 20
 * LOTS   : 1 lot only
 */
class UniversalBTSTReverseController extends Controller
{
    // =========================================================
    //  PAGE
    // =========================================================
    ...
    {
        $pageTitle = 'Universal BTST — Reverse / Contrarian Scanner';
        return view($this->activeTemplate . 'user.universal-btst-reverse.index', compact('pageTitle'));
    }
    ...
    public function analyze(Request $request)
    {
        try {
            ...
            foreach ($tradeDates as $date) {
                ...
                foreach ($activeSymbols as $symbol) {
                    try {
                        ...
                        // ── REVERSE THE SIGNAL ─────────────────────────────
                        // Normal: BULLISH → CE, BEARISH → PE
                        // Reverse: BULLISH → PE, BEARISH → CE
                        $optionType = $signal === 'BULLISH' ? 'PE' : 'CE';
                        ...
                        Log::error("UniversalBTSTReverse ({$symbol}/{$date}): " . $e->getMessage());
                    }
                }
            }
            ...
            Log::error('UniversalBTSTReverse Error: ' . $e->getMessage() . "\n" . $e->getTraceAsString());
            return response()->json(['success' => false, 'message' => 'Error: ' . $e->getMessage(), 'data' => []], 500);
        }
    }

    // =========================================================
    //  BLENDED BUY PRICE  (identical to original)
    // =========================================================

    private function getBlendedBuyPrice(string $symbol, string $date, string $optionType, float $spotPrice, ?string $expiry): float
    {
    ...
    // =========================================================
    //  BLENDED SELL PRICE  (identical to original)
    // =========================================================

    private function getBlendedSellPrice(string $symbol, string $nextDate, string $optionType, ?string $expiry, float $spotPrice): float
    {
        $primaryTimes = ['09:20:00', '09:25:00', '09:30:00'];
        $avgs = [];

        foreach ($primaryTimes as $time) {
            $avg = $this->candleOhlcAvg($this->getAtmCandle($symbol, $nextDate, $optionType, $spotPrice, $expiry, $time));
            if ($avg >= 1) $avgs[] = $avg;
        }

        if (count($avgs) >= 2) return round(array_sum($avgs) / count($avgs), 2);
    ...
    private function getAtmCandle(string $symbol, string $date, string $optionType, float $spotPrice, ?string $expiry, string $time)
    {
        ...
        return (clone $base)->whereNotNull('strike')
    ...
    private function getPrevDayExpiry(string $symbol, string $prevDate, string $currentExpiry): ?string
    {
        ...
        return OptionOhlcData::where('base_symbol', $symbol)
            ->whereIn('instrument_type', ['CE','PE'])
            ->whereDate('trade_date', $prevDate)
            ->whereNotNull('expiry_date')->where('is_missing', 0)
            ->orderBy('expiry_date')
            ->value(DB::raw('DATE(expiry_date)'));
    }
