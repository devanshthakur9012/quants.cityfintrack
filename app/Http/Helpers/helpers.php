...
function allTradeSymbols()
{
    ...
    try {
        ...
        foreach ($data as $vl) {
            // Get the key dynamically instead of hardcoding
            $tableName = array_values((array)$vl)[0]; 
        ...
        \Log::error("Failed to fetch tables from mysql_rm: " . $e->getMessage());
        // Optionally: return a default/fallback list
    }
...
function portfolioName($name) {
    $default = [
        'PF_1' => 'Directional',
        'PF_2' => 'Bi Directional',
        'Portfolio-Futures-Direct' => 'Futures Direct',
        'Portfolio-Options-Opposite' => 'Options Opposite',
        'Portfolio-Futures-Opposite' => 'Futures Opposite',
    ];
...
function calculatePyramids($numbertodivise, $no){
    $intnumber = intval($numbertodivise / $no); // 0.5
    $rem = $numbertodivise % $no; // 1
    $array = [];
    for($i=1; $i <= $no; $i++) {
        if($i == $no) {
            $array[] = $intnumber + $rem;
