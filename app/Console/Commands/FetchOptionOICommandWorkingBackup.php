...
class FetchOptionOICommandWorkingBackup extends Command
{
    protected $signature = 'options:fetch-oi-old 
    ...
    /**
     * ✅ CORRECTED: Calculate and store PE/CE Ratio Analysis with STANDARD market logic
     */
    private function calculateAndStorePECEAnalysis(
        ...
        $futOIChangePct = $futAnalysis['oi']['daily_oi_change_pct'] ?? 0;

        // Calculate PE/CE Ratio
        // $peCeRatio = $ceOI > 0 ? round($peOI / $ceOI, 2) : 0;
        $peCeRatio = $peOI > 0 ? round($ceOI / $peOI, 2) : 0;

        // ✅ STANDARD MARKET LOGIC (NOT REVERSED):
        
        if ($peCeRatio > 1.2) {
            // More PE OI than CE OI = More Put Buying
            $oiInterpretation = 'Put Writing';
            $optionsSentiment = 'Bullish';  // ✅ Put buyers expect upside
            $baseAction = 'BUY CE';  // Buy calls when bullish
            
        } elseif ($peCeRatio < 0.8) {
            // More CE OI than PE OI = More Call Buying
            $oiInterpretation = 'Call Writing';
            $optionsSentiment = 'Bearish';  // ✅ Excess call buying indicates bearishness
            $baseAction = 'BUY PE';  // Buy puts when bearish
        ...
        try {
            ...
            OptionStrike::updateOrCreate(
                ...
                [
                    ...
                    'market_bias' => $oiAnalysis['market_bias'] ?? null,  // ✅ ADDED
                    // ✅ Close price fields
