...
class ZerodhaAutoTradingHelper
{
    private $kite;
    ...
    //         $todayCandles = $this->getTodayCandles($future->trading_symbol, '5minute', $testDate);
    ...
    private function checkAndCreateOrder(ZerodhaAutoConfig $config, FuturesMonitored $future, $testDate = null)
    {
        try {
            ...
            $validSyncPoint = $this->getNextValidSyncPoint($recentSyncPoints, $todayOrders, $future->trading_symbol);
    ...
    /**
     * ✅ Filter sync points to only include recent ones (within last 10 minutes)
     * This prevents old signals from being triggered after fixing bugs/errors
     */
    private function filterRecentSyncPoints($allSyncPoints)
    {
        ...
        $cutoffTime = Carbon::now()->subMinutes(10); // Only signals from last 10 minutes
        ...
        if (count($recentSyncPoints) < count($allSyncPoints)) {
            ...
            Log::info("✅ [FILTER] Remaining recent sync points: " . count($recentSyncPoints));
        }
    ...
    /**
     * Find synchronization points based on strategy (SUPERTREND or VWAP)
     */
    ...
    /**
     * Place pyramid orders with freeze quantity handling
     */
    ...
    //     // Get freeze limit for this symbol
    ...
    //         // Handle freeze quantity
    //         if ($freezeLimit && $pyramidQty > $freezeLimit) {
    //             Log::info("🔄 [FREEZE] Pyramid qty {$pyramidQty} exceeds freeze limit {$freezeLimit}, splitting orders");
                
    //             $numOrders = ceil($pyramidQty / $freezeLimit);
    //             $remainingQty = $pyramidQty;
                
    //             for ($i = 0; $i < $numOrders; $i++) {
    //                 $qtyToPlace = min($freezeLimit, $remainingQty);
                    
    //                 Log::info(sprintf(
    //                     "📦 [FREEZE] Placing split order %d/%d: %d lots",
    //                     (int) ($i + 1),
    //                     (int) $numOrders,
    //                     (int) $qtyToPlace
    //                 ));
                    
    //                 $this->placeKiteOrder($order, $qtyToPlace, $price, $instrument);
                    
    //                 $remainingQty -= $qtyToPlace;
                    
    //                 // Small delay between split orders (2 seconds)
    //                 if ($i < $numOrders - 1) {
    //                     sleep(2);
    //                 }
    //             }
    ...
    /**
     * Place pyramid orders with freeze quantity handling
     * ✅ FIXED: Now correctly calculates freeze limit based on actual quantity (lots × lot_size)
     */
    // private function placePyramidOrders(ZerodhaAutoOrder $order, $instrument)
    // {
    //     $pyramids = [$order->pyramid_1, $order->pyramid_2, $order->pyramid_3];
    ...
    //     // Get freeze limit for this symbol (this is in LOTS, not quantity)
    //     $baseSymbol = $this->extractBaseSymbol($order->future_symbol);
    //     $freezeLimitLots = self::FREEZE_LIMITS[$baseSymbol] ?? null;
        
    //     // ✅ CRITICAL FIX: Calculate freeze limit in actual quantity (lots × lot_size)
    //     $freezeLimitQuantity = $freezeLimitLots ? ($freezeLimitLots * $instrument->lot_size) : null;
        
    //     Log::info("🔍 [FREEZE] Freeze limit for {$baseSymbol}: {$freezeLimitLots} lots = {$freezeLimitQuantity} quantity (lot_size: {$instrument->lot_size})");
    ...
    //         $actualQuantity = $pyramidQty * $instrument->lot_size;
            
    //         // ✅ Handle freeze quantity - compare ACTUAL QUANTITY not LOTS
    //         if ($freezeLimitQuantity && $actualQuantity > $freezeLimitQuantity) {
    //             Log::info("🔄 [FREEZE] Pyramid qty {$pyramidQty} lots ({$actualQuantity} quantity) exceeds freeze limit {$freezeLimitLots} lots ({$freezeLimitQuantity} quantity), splitting orders");
                
    //             // ✅ Calculate number of split orders based on LOTS
    //             $numOrders = ceil($pyramidQty / $freezeLimitLots);
    //             $remainingLots = $pyramidQty;
                
    //             for ($i = 0; $i < $numOrders; $i++) {
    //                 // ✅ Calculate lots to place (never exceed freeze limit in LOTS)
    //                 $lotsToPlace = min($freezeLimitLots, $remainingLots);
    //                 $qtyToPlace = $lotsToPlace * $instrument->lot_size;
    ...
    //                 // ✅ Place order with LOTS (placeKiteOrder will multiply by lot_size)
    ...
    //         } else {
    //             // Place single order if within freeze limit
    //             Log::info("✅ [FREEZE] Pyramid qty {$pyramidQty} lots ({$actualQuantity} quantity) is within freeze limit, placing single order");
    //             $this->placeKiteOrder($order, $pyramidQty, $price, $instrument);
    //         }
    //     }
    // }
