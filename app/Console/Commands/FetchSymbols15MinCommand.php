...
class FetchSymbols15MinCommand extends Command
{
    protected $signature = 'symbols:fetch-15min 
    ...
    protected $description = 'Fetch 15-minute data with Supertrend+50MA (Event Signals + Position Tracking)';
    ...
    public function handle()
    {
        ...
        try {
            ...
            $this->info("   Strategy: Supertrend + 50 MA (Event-Based Signals)");
    ...
    private function fetchSymbolData(BrokerApi $broker, SymbolMonitored $symbol, BrokerZerodhaHelper $zerodhaHelper)
    {
        ...
        // ✅ FIX: Exclude incomplete candle by fetching up to 1 minute ago
        $toDate = date('Y-m-d H:i:s', strtotime('-1 minute'));
        ...
        if ($insertedCount > 0) {
            ...
            $this->calculatePositions($broker->id, $symbol->symbol, '15minute');
            $this->info("      📊 Positions calculated");
        }
    ...
    private function calculateIndicators($brokerId, SymbolMonitored $symbol, string $interval, IndicatorConfig $config)
    {
        try {
            ...
            // ✅ Calculate Supertrend+MA with EVENT signals
            ...
            try {
                foreach ($supertrendResults as $index => $result) {
                    ...
                    ", [
                        ...
                        $result['supertrend'],
                        $result['direction'],
                        $result['event_signal'], // Store event signal in old column for now
                        $result['event_signal'], // Also in new column
                        $result['ma50'],
    ...
    /**
     * Calculate trade positions from event signals
     * Position = derived state (LONG/SHORT/FLAT)
     */
    private function calculatePositions($brokerId, $symbol, $interval)
    {
        try {
            $records = SymbolData::where('broker_api_id', $brokerId)
                ->where('symbol', $symbol)
            ...
            $previousDayPosition = $this->getPreviousTradingDayPosition(
                $brokerId,
                $symbol,
                $interval,
                $firstTimestamp
            ...
            Log::info("📊 Previous day position for {$symbol}: " . ($previousDayPosition ?? 'FLAT'));

            $currentPosition = $previousDayPosition ?? 'FLAT';
            $updateCount = 0;

            foreach ($records as $record) {
                $eventSignal = $record->supertrend_event_signal;

                // Update position based on event signals
                if ($eventSignal === 'BUY') {
                    $currentPosition = 'LONG';
                } elseif ($eventSignal === 'SELL') {
                    $currentPosition = 'SHORT';
                }
                // null event signal = no change to position

                $record->update(['trade_position' => $currentPosition]);
                $updateCount++;
            }

            Log::info("✅ Updated {$updateCount} records with positions");
    ...
    private function getPreviousTradingDayPosition($brokerId, $symbol, $interval, $currentTimestamp)
    {
        ...
        for ($i = 1; $i <= 10; $i++) {
            ...
            if ($lastRecord) {
                ...
                return $lastRecord->trade_position;
            }
        }
