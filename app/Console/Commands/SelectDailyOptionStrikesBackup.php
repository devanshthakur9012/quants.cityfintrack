...
class SelectDailyOptionStrikesBackup extends Command
{
    protected $signature = 'options:select-daily-strikes-backup
                            {--date= : Specific date (Y-m-d)}
                            {--force : Force recalculation}';

    protected $description = 'Select optimal CE/PE strikes based on highest OI each morning';
    ...
    public function handle()
    {
        ...
        $this->info("🎯 Selecting Daily Option Strikes");
        ...
        try {
            // Get first 15-min candle for each symbol (9:15-9:30)
            $morningCandles = SymbolData::whereDate('timestamp', $tradeDate)
                ->whereTime('timestamp', '>=', '09:15:00')
                ->whereTime('timestamp', '<=', '09:30:00')
            ...
            if ($morningCandles->isEmpty()) {
                $this->warn("⚠️  No morning data found for {$tradeDate->format('Y-m-d')}");
                return 1;
            }
            ...
            foreach ($morningCandles as $tradingSymbol => $candles) {
                try {
                    $firstCandle = $candles->first();
                    $this->info("📊 Processing: {$tradingSymbol}");
                    
                    $this->selectStrikesForSymbol($firstCandle, $tradeDate, 'current');
                    $this->selectStrikesForSymbol($firstCandle, $tradeDate, 'next');
            ...
            Log::error('Daily Strike Selection Error: ' . $e->getMessage());
            return 1;
        }
    }

    private function selectStrikesForSymbol($candle, $tradeDate, $optionSeries)
    {
        ...
        $expiry = $this->getExpiry($baseSymbol, $optionSeries);
        
        // ✅ Calculate days to expiry correctly: Expiry Date - Current Date
        ...
        // ✅ Fetch CE options with LIVE OI and LTP
        $this->info("      🔴 Fetching CE options (LIVE)...");
        $ceOptions = $this->fetchOptionsWithOI($baseSymbol, $ceStrikes, 'CE', $expiry, $brokerId);
        
        // ✅ Fetch PE options with LIVE OI and LTP
        $this->info("      🔴 Fetching PE options (LIVE)...");
        $peOptions = $this->fetchOptionsWithOI($baseSymbol, $peStrikes, 'PE', $expiry, $brokerId);
        ...
        // Get recommendation for selected strikes
    ...
    private function fetchOptionsWithOI($baseSymbol, $strikes, $optionType, $expiry, $brokerId = null)
    {
        $options = [];
        
        // ✅ Get broker from the current candle's broker_api_id
        if ($brokerId) {
            $broker = \App\Models\BrokerApi::find($brokerId);
            
            if ($broker && $broker->hasValidToken()) {
                try {
                    // ✅ Fetch LIVE data from Zerodha API
                    ...
                    foreach ($liveOptions as $liveOption) {
                        // Also get the instrument details from database
        ...
        // ❌ FALLBACK: Use database (will have stale/zero OI)
        $this->warn("      ⚠️  Using database fallback (OI may be stale)");
        ...
        foreach ($strikes as $strike) {
            ...
            if ($option) {
                $options[] = [
                    ...
                    'oi' => $option->oi ?? 0, // ⚠️ Likely 0 or stale
                    'ltp' => $option->last_price ?? null, // ⚠️ May be stale
                    'fair_price' => null,
                    'valuation' => null
                ];
            } else {
    ...
    private function getExpiry($baseSymbol, $series)
    {
        ...
        if ($series === 'next') {
            ...
            return $secondExpiry ? $secondExpiry->expiry : now()->addDays(14);
        }

        $firstExpiry = $query->first();
        return $firstExpiry ? $firstExpiry->expiry : now()->addDays(7);
    }
