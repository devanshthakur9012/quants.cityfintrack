...
class FetchSymbolsHistoricalCommand extends Command
{
    protected $signature = 'symbols:fetch-historical 
    ...
    protected $description = 'Fetch historical data with Supertrend+50MA (Event Signals + Position Tracking)';
    ...
    public function handle()
    {
        try {
            ...
            $this->info("   Strategy: Supertrend + 50 MA (Event-Based)\n");
    ...
    private function fetchHistoricalData(BrokerApi $broker, SymbolMonitored $symbol, BrokerZerodhaHelper $zerodhaHelper, $fromDate, $toDate, $interval)
    {
        ...
        if ($insertedCount > 0) {
            ...
            $this->calculatePositions($broker->id, $symbol->symbol, $interval, $fromDate, $toDate);
        }
    ...
    /**
     * Calculate positions for historical date range
     */
    private function calculatePositions($brokerId, $symbol, $interval, $fromDate, $toDate)
    {
        try {
            ...
            // Get position from day BEFORE the historical range
            ...
            Log::info("✅ Historical positions: {$updateCount} records updated for {$symbol}");

        } catch (Exception $e) {
            Log::error("Error calculating historical positions: " . $e->getMessage());
        }
    }
