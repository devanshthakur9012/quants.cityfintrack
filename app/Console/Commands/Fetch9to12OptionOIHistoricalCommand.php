...
/**
 * Backfill historical intraday OI + IV + Close into option_strike_9to12.
 * Open  = 9:15 AM candle
 * Current = 12:15 PM candle
 *
 * Usage:
 *   php artisan options:fetch-9to12-oi-historical --from=2025-01-01 --to=2025-01-31
 *   php artisan options:fetch-9to12-oi-historical --from=2025-01-15 --symbol=NIFTY
 *   php artisan options:fetch-9to12-oi-historical --from=2025-01-15 --broker=2 --debug
 */
class Fetch9to12OptionOIHistoricalCommand extends Command
{
    protected $signature = 'options:fetch-9to12-oi-historical
                            {--from= : From date (Y-m-d)}
                            {--to= : To date (Y-m-d)}
                            {--broker= : Specific broker ID}
                            {--symbol= : Specific underlying symbol}
                            {--force : Process all dates including holidays}
                            {--debug : Show detailed debug information}';

    protected $description = 'Backfill historical INTRADAY OI + IV + Close (9:15 AM → 12:15 PM) into option_strike_9to12';
    ...
    public function handle(): int
    {
        try {
            ...
            $this->info('🚀 Starting Historical 9:15 AM → 12:15 PM OI Backfill');
            ...
            $this->info("\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
            $this->info('✅ Historical 9to12 OI Backfill Completed!');
            $this->info("   Total Processed: {$totalProcessed} | Total Failed: {$totalFailed}");
            $this->info("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
            ...
            Log::error('Historical 9to12 OI fetch error: ' . $e->getMessage());
            return 1;
        }
    }
    ...
    private function processBrokerHistorical(BrokerApi $broker, string $fromDate, string $toDate): array
    {
        $success = 0;
        $failed  = 0;
    ...
    private function processDaySymbols(BrokerApi $broker, BrokerZerodhaHelper $zerodhaHelper, string $date): array
    {
        ...
        foreach ($validSymbols as $futSymbol) {
            try {
                ...
                if (!$spotPrice) {
                    if ($this->option('debug')) $this->warn("   ║    ❌ {$baseSymbol}: No price data");
                    $failed++;
                    continue;
                }
                ...
                Log::error("Historical 9to12 failed: {$baseSymbol}", ['date' => $date, 'error' => $e->getMessage()]);
                $failed++;
            }
        }
    ...
    private function fetchFut(BrokerApi $broker, $futSymbol, BrokerZerodhaHelper $zh, string $date, string $baseSymbol, float $spotPrice, int $atmStrike): ?array
    {
        try {
            ...
            $od = $this->fetchCandle($zh, $instrument->instrument_token, $date, $this->openTime);
            $cd = $this->fetchCandle($zh, $instrument->instrument_token, $date, $this->currentTime);
            ...
            $openCl      = (float) ($od['close'] ?? 0);
            $curCl       = (float) ($cd['close']  ?? 0);
            ...
            Log::error("Historical 9to12 FUT failed: {$baseSymbol}", ['error' => $e->getMessage()]);
            return null;
        }
    }
    ...
    private function fetchCE(BrokerApi $broker, string $baseSymbol, BrokerZerodhaHelper $zh, string $date, int $atmStrike, int $interval, float $spotPrice): ?array
    {
        try {
            ...
            Log::error("Historical 9to12 CE failed: {$baseSymbol}", ['error' => $e->getMessage()]);
            return null;
        }
    }
    ...
    private function fetchPE(BrokerApi $broker, string $baseSymbol, BrokerZerodhaHelper $zh, string $date, int $atmStrike, int $interval, float $spotPrice): ?array
    {
        try {
            ...
            Log::error("Historical 9to12 PE failed: {$baseSymbol}", ['error' => $e->getMessage()]);
            return null;
        }
    }
    ...
    private function getTradingDays(string $fromDate, string $toDate): array
    {
        $dates = [];
        $cur   = Carbon::parse($fromDate);
        $end   = Carbon::parse($toDate);

        while ($cur->lte($end)) {
            $date            = $cur->format('Y-m-d');
            $isSpecialSunday = ($date === '2026-02-01');
            $isHoliday       = DB::table('market_holidays')->where('market_name', 'NSE')->where('holiday_date', $date)->exists();

            if ($this->option('force') || (($isSpecialSunday || !$cur->isWeekend()) && !$isHoliday)) {
                $dates[] = $date;
            }
            $cur->addDay();
        }
