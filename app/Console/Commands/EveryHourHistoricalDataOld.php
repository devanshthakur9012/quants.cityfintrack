...
class EveryHourHistoricalDataOld extends Command
{
    use AngelApiAuth;

    protected $signature = 'options:historical-one-hour-old {--date= : Specific date to process (Y-m-d format)}';
    ...
    private function fetchInstrumentData(string $type, ?string $token, ?string $exchange, string $date): array
    {
        ...
        return Cache::remember($cacheKey, self::CACHE_TTL, function () use ($type, $token, $date, $exchange) {
            ...
            return $this->fetchHistoricalAndOI_ONEHOUR($token, $date, $exchange);
        });
    }
    ...
    private function insertHistoricalData(OptionsChain $chain, string $date, array $currentData, array $calculations): void
    {
        ...
        DB::table('historical_one_hours')->insert($insertData);
    }
