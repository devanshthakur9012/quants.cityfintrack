...
class MCXHistoricalData extends Command
{
    use AngelApiAuth;

    protected $signature = 'options:mcx-historical-data {--date= : Specific date to process (Y-m-d format)}';
    protected $description = 'Fetch mcx-historical-data & OI data for all CE/PE/Future in options chain after market close';

    private const CACHE_TTL = 900; // 15 minutes
    private const API_DELAY = 1; // 1 second delay between API calls
    private const DEFAULT_EXCHANGE = 'MCX';
    ...
    public function handle(): int
    {
        ...
        $chains = $this->getMCXOptionChains();
    ...
    private function getMCXOptionChains()
    {
        return MCXOptionChain::select([
            'id', 'underlying',
    ...
    private function processOptionChain(MCXOptionChain $chain, string $date): array
    {
    ...
    private function fetchAllInstrumentData(MCXOptionChain $chain, string $date): array
    {
        return [
    ...
    private function fetchInstrumentData(string $type, ?string $token, ?string $exchange, string $date): array
    {
        ...
        $cacheKey = "mcx_historical_options:{$type}:{$token}:{$date}";
        ...
        return Cache::remember($cacheKey, self::CACHE_TTL, function () use ($type, $token, $date, $exchange) {
            ...
            dd($token, $date, $exchange, $this->fetchHistoricalAndOI($token, $date, $exchange));
            return $this->fetchHistoricalAndOI($token, $date, $exchange);
        });
    }
    ...
    private function getDataFromDatabase(string $type, string $token, string $date): ?array
    {
        ...
        $data = DB::table('m_c_x_historicals')
    ...
    private function fetchPreviousData(MCXOptionChain $chain): array
    {
        return [
    ...
    private function getLatestDataBySymbol(string $column, ?string $symbol): ?object
    {
        ...
        return DB::table('m_c_x_historicals')
            ->where($column, $symbol)
            ->latest()
            ->first();
    }
    ...
    private function insertHistoricalData(MCXOptionChain $chain, string $date, array $currentData, array $calculations): void
    {
        // Current OI values
        ...
        DB::table('m_c_x_historicals')->insert($insertData);
    }
