...
class FetchFutures5MinCommand extends Command
{
    protected $signature = 'futures:fetch-5min 
                            {--symbol= : Specific symbol to fetch} 
                            {--force : Force fetch even on holidays}
                            {--backfill : Fetch full day data from 9:15 AM}';

    protected $description = 'Fetch 5-minute futures data with all indicators (Supertrend, Donchian, RSI, MACD, VWAP)';
    ...
    public function handle()
    {
        ...
        if (!$this->option('force')) {
            if ($dayName == "Saturday" || $dayName == "Sunday") {
                ...
                Log::info("5-min futures fetch skipped: Weekend");
                return 0;
            }
            ...
            if ($isHoliday) {
                ...
                Log::info("5-min futures fetch skipped: Holiday");
                return 0;
            }
        }
        ...
        try {
            ...
            if ($futures->isEmpty()) {
                $this->info('No 5-min futures to fetch');
                return 0;
            }

            $this->info("📊 Fetching 5-min data for " . $futures->count() . " contract(s)...\n");

            foreach ($futures as $future) {
                $this->info("Processing: {$future->trading_symbol}");
                $this->fetchFuturesData($future, '5minute');
            }

            $this->info("\n✅ 5-minute futures fetch completed!");
            return 0;

        } catch (Exception $e) {
            $this->error("Error: " . $e->getMessage());
            Log::error('5-Min Futures Fetch Error: ' . $e->getMessage());
            return 1;
        }
    }
