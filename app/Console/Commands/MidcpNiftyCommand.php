...
use App\Models\MidcpNifty;
use Carbon\Carbon;
use DateTime;

class MidcpNiftyCommand extends Command
{
    use AngelApiAuth;
    /**
     ...
     * @var string
     */
    protected $signature = 'midcpnifty:every_minute';
    ...
    public function handle(){
        ...
        if(!in_array($currentDate,$marketHolidays)){
            ...
            if($this->isBetween915AMto1130PM()){
                ...
                $LeftmarketData = MidcpNifty::whereNotIn('token_ce',$NfoToken)->orwhereNotIn('token_pe',$NfoToken)->whereDate('created_at', '=', date('Y-m-d'))->groupBy('token_ce')->groupBy('token_pe')->get();
