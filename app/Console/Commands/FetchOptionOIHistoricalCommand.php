...
{
    protected $signature = 'options:fetch-oi-historical 
    ...
    protected $description = 'Fetch historical OI + IV + Close Price data with NEW CE/PE OI Change signals';
    ...
    public function handle()
    {
        try {
            ...
            $this->info("🚀 Starting Historical Daily OI + IV + Close Price Fetch with NEW CE/PE OI Logic");
            $this->info("   From: {$fromDate}");
            $this->info("   To: {$toDate}");
            $this->info("   EOD Time: 3:15 PM");
            $this->info("   Signals: NEW CE/PE OI Change Analysis + BTST");
            $this->newLine();
            ...
            $this->info("\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
            $this->info("✅ Historical Daily OI + IV + Close + NEW CE/PE Signals Completed!");
            ...
            Log::error('Historical Daily OI+IV+Close+NEW CE/PE Fetch Error: ' . $e->getMessage());
            return 1;
        }
    }
    ...
    private function processDailyOIAndIV(BrokerApi $broker, BrokerZerodhaHelper $zerodhaHelper, string $date)
    {
        ...
        foreach ($validSymbols as $futSymbol) {
            try {
                ...
                // ✅ Calculate NEW CE/PE OI Change Signals + BTST
                if ($futAnalysis && $ceAnalysis && $peAnalysis) {
                    ...
                    // ✅ NEW: Calculate CE/PE OI Change Analysis (replaces old PE/CE Ratio logic)
                    $this->calculateAndStoreCEPEOIAnalysis($broker, $baseSymbol, $date, $futAnalysis, $ceAnalysis, $peAnalysis);
                    
                    $this->info("   ║    ✓ {$baseSymbol}: OI + IV + Close + BTST + NEW CE/PE signals stored");
                    $success++;
                } else {
                    $failed++;
                }
    ...
    /**
     * ✅ ALIGNED: Get OI Signal based on CE% and PE% changes (MATCHES FRONTEND)
     * 
     * Logic:
    ...
    /**
     * ✅ NEW: Calculate and store CE/PE OI Change Analysis (REPLACES OLD PE/CE RATIO LOGIC)
     * Uses clean CE%/PE% logic from OIChangeAnalysisController
     */
    private function calculateAndStoreCEPEOIAnalysis(
        BrokerApi $broker,
        string $baseSymbol,
        string $date,
        array $futAnalysis,
        ...
        // ✅ Calculate PE/CE Ratio (kept for reference, but not primary signal anymore)
        $ceOI = $ceAnalysis['oi']['daily_oi'] ?? 0;
        $peOI = $peAnalysis['oi']['daily_oi'] ?? 0;
        $peCeRatio = $ceOI > 0 ? round($peOI / $ceOI, 2) : 0;

        // ✅ Update FUT record with NEW CE/PE OI analysis
        OptionStrike::where('broker_api_id', $broker->id)
            ...
            ->update([
                ...
                // Keep PE/CE ratio for reference
        ...
        if ($this->option('debug')) {
            ...
            $this->info("      📊 PE/CE Ratio: {$peCeRatio} (reference only)");
        }
    }
        ...
        try {
            ...
            OptionStrike::updateOrCreate(
                ...
                [
                    ...
                    // ✅ Close price fields (average of 3 strikes)
