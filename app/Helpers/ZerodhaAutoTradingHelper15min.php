...
class ZerodhaAutoTradingHelper15min
{
    private $kite;
    ...
    private function checkAndCreateOrder(ZerodhaAutoConfig $config, FuturesMonitored $future, $testDate = null)
    {
        try {
            ...
            // ✅ ADD THIS: Filter to recent sync points (within last 30 minutes)
            $recentSyncPoints = $this->filterRecentSyncPoints($allSyncPoints);

            if (empty($recentSyncPoints)) {
                Log::info("⚠️ [CHECK] No recent sync points found (all older than 30 minutes)");
                return;
            }

            // Log::info("✅ [CHECK] Found " . count($recentSyncPoints) . " recent sync points");

            // ✅ CHANGE THIS: Use recentSyncPoints instead of allSyncPoints
    ...
    private function filterRecentSyncPoints($allSyncPoints)
    {
        ...
        if (count($recentSyncPoints) < count($allSyncPoints)) {
            ...
            Log::info("🕐 [FILTER] Filtered out {$filtered} old sync points (older than 30 minutes)");
