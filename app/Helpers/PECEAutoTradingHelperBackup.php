...
/**
 * PECEAutoTradingHelperBackup
 *
 * EXPIRY DAY FIX (all three bugs corrected):
 *
 * Bug 1 — resolveDataSeries() returned March on expiry day
 *   Because: first($d >= currentDate) matches today's expiry, then
 *            in_array($currentDate, $allSeries) forces $active = today.
 ...
 *   Because: whereDate('expiry_date', '>=', $date) includes today's expiry
 *            which is the smallest and gets picked first.
 *   Fix:     resolveActiveExpiry() wraps getNearestExpiryForDate() — if the
 *            resolved expiry == today, shift to next series (same logic as
 *            LiveOptionOhlcCollector::resolveExpiriesFor15Min).
 *            Also normalise all DB::raw date returns with substr(,0,10) to
 *            prevent Carbon/PDO-string === comparison silently failing.
 *
 * Bug 3 — prevExpiry on expiry day compared April OI vs March's prev-day OI
 *   Because: getPrevDayExpiry() was called unconditionally.
 ...
 */
class PECEAutoTradingHelperBackup
{
    // EOD timeframe
    const OPEN_TIME_HOUR    = 15;   // previous day
    const OPEN_TIME_MINUTE  = 0;    // 15:00 previous day
    const CLOSE_TIME_HOUR   = 14;   // today
    const CLOSE_TIME_MINUTE = 45;   // 14:45 today
    ...
    private function throttleRequest(int $brokerId): void
    {
        if (isset($this->lastRequestTime[$brokerId])) {
            ...
            if ($elapsedMs < $this->minMsBetweenRequests) {
                $sleepUs = ($this->minMsBetweenRequests - $elapsedMs) * 1000;
                usleep($sleepUs);
            }
        }
        ...
        foreach ($chunks as $chunk) {
            ...
            while ($attempt < $maxRetries) {
                try {
                    ...
                    Log::info(sprintf(
                        "EOD prefetchLTPs broker=%d | fetched %d symbols in batch",
    ...
    public function processSignals($testDate = null)
    {
        try {
            ...
            // ── FIXED: resolveDataSeries now skips today's expiry on expiry day ──
    ...
    {
        try {
            Log::info('=== EOD: Starting PE/CE Order Placement ===');
    ...
    // =========================================================
    //  SERIES RESOLUTION  — FIXED
    // =========================================================

    /**
     * FIX: On expiry day, the old code forced $active = $currentDate
     * (because today's expiry IS in the $allSeries list), causing
     * aggregateSignalsFromOhlc to query the expiring (near-zero) contract.
     *
     * New logic mirrors resolveInstrumentSeries exactly:
     *   - If today IS an expiry date → use '>' comparator → skip to next series
     *   - Otherwise use '>=' → pick the current active series
     *
     * This ensures dataSeries always points to the series the collector
     * actually stored meaningful data for.
     */
    private function resolveDataSeries(string $currentDate): ?string
    {
        // Check if today itself is an expiry date in the data
        $isTodayExpiry = OptionOhlcData::whereIn('instrument_type', ['CE', 'PE'])
            ->whereNotNull('expiry_date')
            ->whereDate('expiry_date', $currentDate)
            ->whereDate('trade_date', $currentDate)   // data was actually collected today
            ->where('is_missing', 0)
            ->exists();

        // On expiry day: skip today's expiry → pick next series
        // Normal day   : pick nearest expiry >= today
        ...
        // Ultimate fallback: any future expiry in the DB
        $series = OptionOhlcData::whereIn('instrument_type', ['CE', 'PE'])
            ->whereNotNull('expiry_date')
            ->whereDate('expiry_date', '>', $currentDate)
            ->orderBy('expiry_date', 'ASC')
    ...
    // =========================================================
    //  EXPIRY HELPERS  — FIXED (mirrors OIIVAutoController fix)
    // =========================================================

    /**
     * Get the raw nearest expiry date for a symbol on a given date.
     * Returns Y-m-d string, normalised with substr() to avoid Carbon vs
     * plain-string comparison failures (PDO casting inconsistency).
     *
     * NOTE: On expiry day this returns TODAY's expiry.
     * Callers that need next-series behaviour must use resolveActiveExpiry().
     */
    ...
    /**
     * FIX: On expiry day, shift from the expiring contract to the next series.
     *
     * The collector (LiveOptionOhlcCollector) does NOT store expiring-contract
     * data on expiry day — it stores next-series data instead. So querying
     * the expiring contract's CE/PE OI returns 0 → symbol gets dropped.
     *
     * This method mirrors LiveOptionOhlcCollector::resolveExpiriesFor15Min():
     *   - If nearestExpiry == today → shift to next series
    ...
    private function resolveActiveExpiry(string $symbol, string $date): ?string
    {
        ...
        if ($expiryDate === $tradeDate) {
            // Today is expiry day — shift to next series
            ...
            // Fallback: any future expiry in DB
            $next = OptionOhlcData::where('base_symbol', $symbol)
                ->whereIn('instrument_type', ['CE', 'PE'])
                ->whereNotNull('expiry_date')
                ->whereDate('expiry_date', '>', $expiryDate)
    ...
    // =========================================================
    //  CORE: AGGREGATE SIGNALS FROM option_ohlc_data  — FIXED
    // =========================================================

    /**
     * FIX summary:
     *   1. resolveActiveExpiry() replaces getNearestExpiryForDate() for OI queries
     *      → on expiry day picks April instead of expiring March contract
     *   2. $isExpiryDay detected via normalised substr() comparison
     *   3. prevExpiry = null on expiry day → ceOiPct/peOiPct = 0 (correct:
     *      April series is brand new, no prev-day baseline exists)
     *   4. Detailed logging includes is_expiry_day flag for debugging
     */
    private function aggregateSignalsFromOhlc(string $currentDate, string $prevDate, string $dataSeries): array
    {
        ...
        foreach ($futCandles as $symbol => $futCandle) {
            ...
            // ── Expiry-day-aware expiry resolution ────────────────────────────
            // resolveActiveExpiry() shifts to next series on expiry day.
            // isExpiryDay is used to zero out prevExpiry (new series has no
            // prev-day OI baseline — comparing April OI vs March prev-day OI
            // gives garbage % change and meaningless signals).
            ...
            // On expiry day: new series has no prev-day baseline → prevExpiry = null
            // → ceOiPct = 0, peOiPct = 0 → signal falls back to price direction only
    ...
    private function analyzeAndCreateOrder(
        ...
        try {
            ...
            $lockTime = Carbon::parse($date . ' ' . self::CLOSE_TIME_HOUR . ':' . sprintf('%02d', self::CLOSE_TIME_MINUTE) . ':00', 'Asia/Kolkata');
            ...
            $reason = sprintf(
                "EOD | Rank:%d | Dir:%s | Mode:%s | BUY %s | CE%%:%.2f | PE%%:%.2f | Diff:%.2f | %s | Sentiment:%s | 50MA:DISABLED | ExpiryDay:%s | DataSeries:%s | InstrSeries:%s | Qty:%d | CurExpiry:%s | PrevExpiry:%s",
            ...
            Log::info("EOD Order created! ID:{$order->id} | {$optionDetails['symbol']} | Type:{$finalOptionType} | Strike:{$optionDetails['strike']} | Rank{$rank} | Qty:{$quantity} | InstrSeries:{$instrumentSeries}");
            return true;
    ...
    {
        try {
            Log::info("EOD ORDER Placing: {$order->option_symbol}");
            $broker = $order->broker;
    ...
    {
        $baseForExchange = preg_replace('/\d{2}[A-Z]{3}\d+[CP]E$/i', '', $order->option_symbol);
        $baseForExchange = preg_replace('/\d{2}[A-Z]{3}(FUT)?$/i', '', $baseForExchange);
        $orderExchange   = $this->getExchangeForSymbol(strtoupper($baseForExchange));
        ...
        for ($attempt = 1; $attempt <= $maxRetries; $attempt++) {
            try {
                ...
                Log::info("EOD ORDER Placed! Kite ID: {$result->order_id}");
                ...
                if ($this->isRateLimitError($e) && $attempt < $maxRetries) {
                    $wait = (int) pow(2, $attempt);
                    Log::warning("EOD ORDER rate-limited {$order->option_symbol} — retry {$attempt}/{$maxRetries} in {$wait}s");
                    sleep($wait);
                } else {
                    throw $e;
                }
    ...
    // =========================================================
    //  50MA  (kept for future use — NOT called for orders)
    // =========================================================
    ...
    private function saveToOrderBook(OIIVAutoOrder $order, $orderId, $quantity, $price): void
    {
        try {
            ...
            Log::error("EOD ORDER_BOOK: " . $e->getMessage());
        }
    }
    ...
    private function saveFailedOrder(OIIVAutoOrder $order, $quantity, $price, string $error): void
    {
        try {
            ...
            Log::error("EOD ORDER_BOOK Failed: " . $e->getMessage());
        }
    }
