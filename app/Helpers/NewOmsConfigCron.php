...
class NewOmsConfigCron{
    ...
    public function callKiteApi(object $omsData)
    {
        ...
        if (!$currentLTP) {
            ...
            OmsConfigs::where('id', $omsData->id)->update(['is_api_pushed' => 2]);
    ...
    public function placeOrder()
    {
        ...
        ->chunk(100, function ($omsConfigs) use (&$processedIds, &$errorIds){
            foreach ($omsConfigs as $config) {
                ...
                if ($result == 1) {
                    ...
                    OmsConfigs::where('id', $config->id)->update([
                        'is_api_pushed' => 1,
        ...
        if (!empty($processedIds)) {
            OmsConfigs::whereIn('id', $processedIds)
                ->update([
        ...
        if (!empty($errorIds)) {
            OmsConfigs::whereIn('id', $errorIds)
