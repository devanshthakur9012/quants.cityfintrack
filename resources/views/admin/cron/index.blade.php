                    ...
                    <table class="table--light style--two table bg-white">
                        ...
                        <tbody>
                            @forelse ($crons as $cron)
                                ...
                                <tr>
                                    <td>
                                        {{ __($cron->name) }} @if($cron->logs->where('error','!=',null)->count()) <i class="fas fa-exclamation-triangle text--danger"></i> @endif <br> 
                                        <code>{{ __($cron->alias) }}</code>
                                    </td>
                                    <td>{{ __($cron->schedule->name) }}</td>
                                    <td>@if($cron->next_run) {{ __($cron->next_run) }} <br> {{ diffForHumans($cron->next_run) }} @else -- @endif</td>
                                    <td>@if($cron->last_run) {{ __($cron->last_run) }} <br> {{ diffForHumans($cron->last_run) }} @else -- @endif</td>
                                    <td>
                                        @if($cron->is_running)
                                    ...
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu p-0">
                                                ...
                                                <a href="" data-id="{{ $cron->id }}" data-name="{{ $cron->name }}" data-url="{{ $cron->url }}" data-next_run="{{ $formattedDateTime }}" data-cron_schedule_id="{{ $cron->cron_schedule_id }}" data-default="{{ $cron->is_default }}" class="dropdown-item updateCron"><i class="las la-pen"></i> @lang('Edit')</a>
