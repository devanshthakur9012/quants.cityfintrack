...
<div class="row">
    <div class="col-lg-12">
        <form action="{{route('admin.portfolio-insights.top-losers.remove-stock-portfolio')}}" name="record_frm" id="record_frm" method="post">
            @csrf
                <div class="card responsive-filter-card mb-4">
                    <div class="card-body">
                        <div class="d-flex flex-wrap gap-3">
                            ...
                            <div class="flex-grow-1 align-self-end">
                                <a class="btn btn--primary w-100 h-45" href="{{url('admin/portfolio-insights/top-losers')}}"><i class="las la-sync-alt"></i> Refresh</a>
                            </div>
                        </div>
                    </div>
                </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex justify-content-end flex-wrap gap-2">
                                            {{-- <a href="{{ route('admin.signal.edit', $portfolioTopLoser->id) }}"
                                                class="btn btn-sm btn-outline--primary">
                                                <i class="la la-pencil"></i> @lang('Edit')
                                            </a> --}}
                                            <button class="btn btn-sm btn-outline--danger confirmationBtn"
                                                data-question="@lang('Are you sure to delete this record')?"
                                                data-action="{{ route('admin.portfolio-insights.top-losers.delete') }}"
                                                data-hidden_id="{{ $portfolioTopLoser->id }}">
            ...
            @if ($portfolioTopLosers->hasPages())
...
<script>    
    $("#stock_name").select2({
        ...
        ajax: {
            url: "{{ route('admin.portfolio-insights.top-losers.get-stock-name') }}",
            dataType: "json",
            type: "GET",
...
<script>
    $("#filter_btn").on('click',function(){
        ...
        window.location.href = '{{url("admin/portfolio-insights/top-losers")}}?stock_name='+stock_name;
    });
</script>
