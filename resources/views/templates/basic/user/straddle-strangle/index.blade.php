...
/* ── Header ── */
.ss-hdr{background:linear-gradient(135deg,#071422,#0c1f3a 55%,#071422);
    border:1px solid rgba(0,229,255,.18);border-radius:14px;padding:18px 24px;margin-bottom:16px;
    position:relative;overflow:hidden;}
.ss-hdr::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;
    background:linear-gradient(90deg,transparent,var(--cyan),transparent);}
.ss-hdr h4{color:var(--cyan);margin:0;font-size:18px;font-weight:800;letter-spacing:.5px;}
.ss-hdr p{color:var(--mid);margin:5px 0 0;font-size:11px;line-height:1.7;}
.hpill{display:inline-block;background:rgba(0,229,255,.1);border:1px solid rgba(0,229,255,.25);
    border-radius:5px;padding:2px 9px;font-size:10px;color:var(--cyan);font-weight:700;margin-right:3px;}
...
/* ── Filter bar ── */
.ss-filter{background:linear-gradient(135deg,#0e1d35,#152845);
    border:1px solid rgba(0,229,255,.1);border-radius:11px;
...
.f-sel option{background:#0d1b2e;}
...
.f-load{background:var(--cyan);color:#000;border:none;border-radius:7px;padding:6px 20px;font-weight:900;font-size:12px;cursor:pointer;}
.f-load:hover{background:#33eaff;}
...
.detail-hdr{
    background:linear-gradient(135deg,rgba(0,0,0,.4),rgba(0,229,255,.04));
    border:1px solid rgba(0,229,255,.2);border-radius:12px;
    padding:14px 18px;margin-bottom:14px;
    display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;}
.detail-sym{font-size:22px;font-weight:900;color:var(--cyan);letter-spacing:.5px;}
...
.dm-cyan{background:rgba(0,229,255,.1);color:var(--cyan);border-color:rgba(0,229,255,.25);}
...
.mp-c{background:rgba(0,229,255,.1);border-color:rgba(0,229,255,.25);color:var(--cyan);}
...
.tr-entry{background:rgba(0,229,255,.05) !important;}
...
.sep-pr{border-left:2px solid rgba(0,229,255,.3) !important;}
...
.bg-pr{background:rgba(0,229,255,.018) !important;}
...
.h-pr{color:var(--cyan) !important;}
.h-oi{color:var(--amber) !important;}
.h-ex{color:var(--purple) !important;}

/* Cell styles */
.sym-badge{display:inline-block;background:rgba(0,229,255,.12);border:1px solid rgba(0,229,255,.25);
    border-radius:6px;padding:3px 10px;font-size:12px;font-weight:800;color:var(--cyan);}
.c-atm{font-size:9px;color:var(--amber);font-weight:600;}
.c-exp{font-size:9px;color:var(--dim);margin-top:1px;}
.time-entry{background:rgba(0,229,255,.15);color:var(--cyan);border:1px solid rgba(0,229,255,.35);
...
.c-pr{color:#e0f7fa;font-weight:700;}
...
.c-ltp{color:#e0f7fa;font-weight:700;}
.c-prem{color:var(--cyan);font-weight:800;font-size:12px;}
...
.str-m{background:rgba(0,229,255,.12);color:var(--cyan);border:1px solid rgba(0,229,255,.3);border-radius:4px;padding:2px 7px;font-size:9px;font-weight:700;}
...
.hd-ce,.hd-pe{background:rgba(0,229,255,.1);color:var(--cyan);border:1px solid rgba(0,229,255,.25);border-radius:5px;padding:2px 9px;font-size:10px;font-weight:700;}
...
.spinner{width:32px;height:32px;border:3px solid rgba(255,255,255,.1);border-top:3px solid var(--cyan);border-radius:50%;animation:spin 1s linear infinite;}
...
<div class="container-fluid content-container">
    ...
    <div class="ss-hdr">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
            <div>
                <h4>&#9889; Straddle &amp; Strangle — 1-Hour Candle</h4>
                <p>
                    <span class="hpill hpill-g">&#9679; Entry: Live LTP @ 9:16</span>
                    <span class="hpill hpill-a">&#9660; Running: Latest 1-hr candle</span>
                    <span class="hpill">&#128200; All symbols summary or pick one symbol for every 1-hr interval</span>
            ...
            <div class="d-flex align-items-center gap-2">
                <a href="{{ route('straddle-strangle-15.index') }}" class="btn btn-sm btn-outline-info">&#8594; 15-Min Version</a>
    ...
    <div class="ss-filter">
        ...
        <button class="f-auto" id="auto-btn" onclick="toggleAuto()">&#9654; Auto 60s</button>
...
function toggleAuto(){
    if(autoTimer){ clearInterval(autoTimer); autoTimer=null; $('#auto-btn').text('▶ Auto 60s'); $('#auto-tag').text(''); }
    else{ autoTimer=setInterval(loadData,60000); $('#auto-btn').text('■ Stop'); $('#auto-tag').css('color','#00e676').text('● live'); loadData(); }
}

function clearCache(){
    $.post('{{ route("straddle-strangle.clear-ltp-cache") }}',
...
function loadData(){
    ...
    if(date!==todayStr&&autoTimer){ clearInterval(autoTimer); autoTimer=null; $('#auto-btn').text('▶ Auto 60s'); $('#auto-tag').text(''); }
    ...
    $.ajax({
        url :'{{ route("straddle-strangle.data") }}',
...
function renderSummary(res){
    ...
    $('#strat-strip').html(`
        <span class="${isShort?'sp-short':'sp-long'}">${isShort?'&#9660; SHORT':'&#9650; LONG'} — ${res.strategy_name}</span>
        ...
        <span class="meta-sep">|</span>
        <span style="background:rgba(0,229,255,.1);border:1px solid rgba(0,229,255,.25);color:var(--cyan);border-radius:6px;padding:3px 10px;font-size:11px;font-weight:800;">${res.total} Symbols</span>
        <span style="background:rgba(68,138,255,.1);border:1px solid rgba(68,138,255,.25);color:var(--blue);border-radius:6px;padding:3px 10px;font-size:10px;font-weight:700;">&#9201; 1-Hr</span>
    `).show();
    ...
    const meta=`<div class="tbl-meta">
        ...
        <span class="mp mp-c">Entry: ${res.entry_slot}</span>
        <span class="mp mp-a">Latest: ${latestLabel}</span>
        <span class="mp mp-b">&#9201; 1-Hr</span>
    ...
    const thead=`<thead>
        <tr class="g-hdr">
            ...
            <th colspan="6" class="h-ce sep-ce">&#128200; CE — ${res.ce_type} ${res.ce_pos}</th>
            <th colspan="6" class="h-pe sep-pe">&#128201; PE — ${res.pe_type} ${res.pe_pos}</th>
    ...
    res.data.forEach((s,i)=>{
        ...
        const ceSym=s.ce_trading_symbol
            ?`<div class="c-sn">${s.ce_trading_symbol}</div>`
            :dim(s.ce_symbol_name||'—');
        const peSym=s.pe_trading_symbol
            ?`<div class="c-sn">${s.pe_trading_symbol}</div>`
            :dim(s.pe_symbol_name||'—');
        const lotBadge=s.lot_size&&s.lot_size>1?`<span class="lot-badge">${nInt(s.lot_size)}</span>`:dim(s.lot_size||'—');
        ...
        tbody+=`<tr>
            ...
            <td class="bg-pr">${pnlUnitHtml(s.profit_loss)}</td>
            <td class="bg-pr">${lotBadge}</td>
            <td class="bg-pr">${pnlLotHtml(s.profit_loss_lot,s.lot_size)}</td>
    ...
    $('#ss-wrap').html(`<div class="tbl-card">${meta}<div class="tbl-wrap"><table class="ss-tbl summary">${thead}${tbody}</table></div>${legendBar()}</div>`);
}

// ══════════════════════════════════════════════════════════════════════════════
// MODE 2 — Single symbol every 1-hr interval
// ══════════════════════════════════════════════════════════════════════════════
...
function renderDetail(res){
    ...
    const detHdr=`<div class="detail-hdr">
        <div>
            ...
            <div style="font-size:11px;color:var(--mid);margin-top:4px;">${res.strategy_name} &nbsp;|&nbsp; CE: ${res.ce_txn_type} ${res.ce_pos} &nbsp;|&nbsp; PE: ${res.pe_txn_type} ${res.pe_pos}</div>
        </div>
        <div class="detail-meta">
            <span class="dm dm-cyan">ATM &#8377;${nInt(res.atm_strike)}</span>
            <span class="dm dm-amber">Expiry: ${res.expiry||'—'}</span>
            <span class="dm dm-green">Entry: ${res.entry_slot} &nbsp;|&nbsp; ${isLive?'&#9679; Live LTP':'09:15 Open'}</span>
            <span class="dm dm-cyan">CE Entry: &#8377;${nf(res.ce_price)} &nbsp;|&nbsp; PE Entry: &#8377;${nf(res.pe_price)}</span>
    ...
    const thead=`<thead>
        <tr class="g-hdr">
            ...
            <th colspan="5" class="h-ce sep-ce">&#128200; CE — ${res.ce_txn_type} ${res.ce_pos} ${res.ce_trading_symbol?'('+res.ce_trading_symbol+')':''}</th>
            <th colspan="5" class="h-pe sep-pe">&#128201; PE — ${res.pe_txn_type} ${res.pe_pos} ${res.pe_trading_symbol?'('+res.pe_trading_symbol+')':''}</th>
            <th colspan="5" class="h-pr sep-pr">&#9654; Premium &amp; P&amp;L</th>
            <th colspan="2" class="h-oi sep-oi">&#9889; OI Sentiment</th>
            <th colspan="3" class="h-ex sep-ex">&#9888; Exit Management</th>
        </tr>
    ...
    res.data.forEach(iv=>{
        ...
        tbody+=`<tr class="${rowCls}">
            ...
            <td class="bg-pr">${pnlUnitHtml(iv.profit_loss)}</td>
            <td class="bg-pr">${lotBadge}</td>
            <td class="bg-pr">${pnlLotHtml(iv.profit_loss_lot,iv.lot_size)}</td>
    ...
    $('#ss-wrap').html(detHdr+`<div class="tbl-card"><div class="tbl-wrap"><table class="ss-tbl detail">${thead}${tbody}</table></div>${legendBar()}</div>`);
}
...
function pnlUnitHtml(v){
    ...
    const pos=v>=0;
    return `<strong class="${pos?'c-pos':'c-neg'}">${pos?'+':'-'}&#8377;${nf(Math.abs(v))}</strong>`;
}
...
function pnlLotHtml(vLot,lotSize){
    ...
    const arrow=pos?'&#9650;':'&#9660;';
    const sub=lotSize&&lotSize>1?`<span class="pnl-lot-sub">×${nInt(lotSize)} lots</span>`:'';
...
function txnBadge(t){
    if(!t) return dim('—');
    return t==='Buy'?'<span class="tb-buy">&#9650; BUY</span>':'<span class="tb-sell">&#9660; SELL</span>';
}
...
function legendBar(){
    return `<div class="legend">
        ...
        &nbsp;·&nbsp; <span style="color:var(--cyan);font-weight:700;">&#9650; Cyan row</span> = Entry (09:15)
        ...
        &nbsp;·&nbsp; Click any symbol badge to drill into 1-hr intervals
    </div>`;
}
</script>
@endpush