...
@section('content')
    ...
    <style>
        /* ══════════════════════════════════════════════
           CITYQUANTS — MOMENTUM BREAKOUT SCANNER  v2.1
        ...
        *::after {
        ...
        .mb-anim {
            animation: mbFadeUp .5s ease both;
        }
        ...
        .mb-inst-tab.on-stock {
            background: rgba(38, 166, 154, .1);
            border-color: rgba(38, 166, 154, .3);
            color: var(--c-teal);
        }
        ...
        .mb-inst-tab.on-option {
            background: rgba(171, 71, 188, .1);
            border-color: rgba(171, 71, 188, .3);
            color: var(--c-purple);
        }
        ...
        /* Scan-mode vs history-mode toggling */
        .mb-scan-controls,
        .mb-history-controls {
            display: flex;
            align-items: center;
        ...
        .mb-date-input::-webkit-calendar-picker-indicator {
        ...
        .mb-threshold-select:focus {
            border-color: rgba(125, 255, 0, .45);
        }

        .mb-threshold-select option {
            background: var(--c-panel);
            color: var(--c-text);
        }
        ...
        .mb-stat-card.s-total::after {
            background: var(--c-blue);
        }

        .mb-stat-card.s-ce::after {
            background: var(--c-teal);
        }

        .mb-stat-card.s-pe::after {
            background: var(--c-red);
        }

        .mb-stat-card.s-nt::after {
        ...
        .sep-signal {
            border-left: 1px solid rgba(255, 167, 38, .15) !important;
        }
        ...
        #mbLogicModal .modal-header {
            border-bottom: 1px solid var(--c-border);
            padding: 18px 22px;
        }

        #mbLogicModal .modal-footer {
            border-top: 1px solid var(--c-border);
            padding: 14px 22px;
        }

        #mbLogicModal .modal-body {
            padding: 4px 22px 22px;
        }

        #mbLogicModal .modal-title {
            font-family: var(--f-display);
            font-weight: 800;
            color: #fff;
            font-size: 18px;
        ...
        #mbLogicModal .modal-title span {
            color: var(--c-lime);
        }

        #mbLogicModal .btn-close {
            filter: invert(1);
            opacity: .55;
        }

        #mbLogicModal .btn-close:hover {
            opacity: .9;
        }
        ...
        #mbLogicModal .table> :not(caption)>*>* {
            background: transparent;
        ...
        #mbLogicModal .mb-flow-step:last-child {
            border-bottom: none;
        }

        #mbLogicModal .mb-flow-num {
            width: 20px;
            height: 20px;
            border-radius: 50%;
            background: var(--c-lime-dim);
        ...
        <div class="mb-hero mb-anim">
            <div class="mb-hero-left">
                ...
                <p>
                    ...
                    Pick a symbol to pull its breakout history across any date range.
                </p>
            </div>
        ...
        <div class="mb-filter-bar">
            <div class="mb-filter-inner">
                ...
                <div class="mb-inst-tabs">
                    <button class="mb-inst-tab on-stock" data-inst="stock" onclick="mbSetInst('stock',this)">
                        <i class="las la-chart-line"></i> Stock EQ
                    </button>
                    <button class="mb-inst-tab" data-inst="fut" onclick="mbSetInst('fut',this)">
                        <i class="las la-fire"></i> Futures
                    </button>
                    <button class="mb-inst-tab" data-inst="option" onclick="mbSetInst('option',this)">
                ...
                <select id="mb-sym" class="mb-sym-select" onchange="mbSymbolChanged()">
                    <option value="ALL">— All Symbols —</option>
                </select>

                <div class="mb-sep"></div>

                {{-- Single-date scan mode (default, symbol = ALL) --}}
                <div class="mb-scan-controls" id="mb-scan-controls">
                    <span class="mb-filter-label">Date</span>
                    <div class="mb-date-wrap">
                        <button class="mb-date-nav" onclick="mbShiftDate(-1)">‹</button>
                        <input type="date" id="mb-date" class="mb-date-input" value="{{ now()->toDateString() }}"
                            max="{{ now()->toDateString() }}" onchange="mbScan()">
                        <button class="mb-date-nav" onclick="mbShiftDate(1)">›</button>
                ...
                {{-- Date-range history mode (shown once a specific symbol is picked) --}}
                <div class="mb-history-controls" id="mb-history-controls">
                    <span class="mb-filter-label">From</span>
                    <input type="date" id="mb-hist-start" class="mb-date-input" max="{{ now()->toDateString() }}">
                    <span class="mb-filter-label">To</span>
                    <input type="date" id="mb-hist-end" class="mb-date-input" max="{{ now()->toDateString() }}"
                ...
                <select id="mb-threshold" class="mb-threshold-select" onchange="mbTriggerCurrent()">
                ...
                <label class="mb-nt-wrap">
                    <input type="checkbox" id="mb-show-nt" onchange="mbTriggerCurrent()">
                    No-Trade
                </label>

                <button class="mb-scan-btn" id="mb-scan-btn" onclick="mbScan()">
                    <i class="las la-search"></i> Scan
                </button>
                <button class="mb-scan-btn" id="mb-hist-btn" style="display:none;" onclick="mbLoadHistory()">
                    <i class="las la-history"></i> Load History
                </button>
                <button class="mb-reset-btn" onclick="mbReset()">↺ Reset</button>

                <div class="mb-filter-right">
                    <button type="button" class="mb-logic-btn" data-bs-toggle="modal" data-bs-target="#mbLogicModal">
        ...
        <div class="mb-content">
            ...
            <div class="mb-stats mb-anim">
                <div class="mb-stat-card s-total">
                    <div class="mb-stat-label" id="st-total-label">Total Records</div>
            ...
            <div class="mb-card mb-anim d1">
                <div class="mb-card-header">
                    <div class="mb-card-title">
                        ...
                        <span id="mb-card-title-text">Breakout Signals</span>
                    </div>
...
══════════════════════════════════════════════════════════ --}}
        <div class="modal fade" id="mbLogicModal" tabindex="-1" aria-labelledby="mbLogicModalLabel"
            aria-hidden="true">
            <div class="modal-dialog modal-dialog-scrollable modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="mbLogicModalLabel">⚡ Momentum Breakout <span>— Complete Signal
                                Logic</span></h5>
                    ...
                    <div class="modal-body">
                        ...
                        <h6>Overview</h6>
                        <p>
                            Momentum Breakout is a single-shot threshold scanner. For every symbol and every
                            trading day it walks the 15-minute candles in order from market open and fires
                            the moment price moves far enough away from the day's opening price — one signal
                            per symbol per day, no re-entry and no reversal tracking after that.
                        </p>

                        <h6>Instruments &amp; Data</h6>
                            ...
                            <table class="table table-sm">
                                <thead>
                                    <tr>
                                        <th>Mode</th>
                                        <th>Table</th>
                                        <th>What moves the trigger</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
                                        <td style="color:#fff;">Stock EQ</td>
                                        <td><code>cp_stock_ohlc_15min</code></td>
                                        <td>The stock's own candle close vs. its own day open</td>
                                    </tr>
                                    <tr>
                                        <td style="color:#fff;">Futures</td>
                                        <td><code>cp_fut_ohlc_15min</code></td>
                                        <td>The futures candle close vs. the futures day open</td>
                                    </tr>
                                    <tr>
                                        <td style="color:#fff;">Options</td>
                                        <td><code>cp_option_ohlc_15min</code> (ATM CE only)</td>
                                        <td>The <strong style="color:#fff;">underlying future price</strong> vs. its day
                                            open — the ATM call is bought as a proxy once that threshold is crossed</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                        <p>
                            In Options mode the breakout is timed off the future's own price movement
                            (falling back to the option's own open if a future price isn't available on
                            that row), while the reported signal price is the ATM call's own premium at
                            that candle — the underlying's move decides <em>when</em>, the option's premium
                            is <em>what you'd pay</em>.
                        </p>

                        <h6>Detection Rule</h6>
                        <div class="mb-flow-step">
                            <span class="mb-flow-num">1</span>
                            <div><strong style="color:#fff;">Day Open</strong> — the very first candle's open (09:15) for
                                that symbol and day.</div>
                        </div>
                        <div class="mb-flow-step">
                            <span class="mb-flow-num">2</span>
                            <div><strong style="color:#fff;">Change %</strong> — for each candle in sequence: <code>(price
                                    − day open) / day open × 100</code>.</div>
                        </div>
                        <div class="mb-flow-step">
                            <span class="mb-flow-num">3</span>
                            <div><strong style="color:#fff;">Trigger</strong> — change % ≥ threshold → <code>BUY_CE</code>;
                                change % ≤ −threshold → <code>BUY_PE</code>.</div>
                        </div>
                        <div class="mb-flow-step">
                            <span class="mb-flow-num">4</span>
                            <div><strong style="color:#fff;">First win</strong> — the first candle to cross the threshold
                                (in either direction) becomes that symbol's signal for the day; the scan stops there.</div>
                        </div>
                        <div class="mb-flow-step">
                            <span class="mb-flow-num">5</span>
                            <div><strong style="color:#fff;">No Trade</strong> — if no candle ever crosses the threshold,
                                the day is marked <code>NO_TRADE</code> (only shown when the "No-Trade" toggle is on).</div>
                        </div>

                        <h6>Threshold Options</h6>
                        <p>
                            <span class="badge-w">0.5%</span> <span class="badge-w">0.75%</span>
                            <span class="badge-w">1.0%</span> <span class="badge-w">1.25%</span>
                            <span class="badge-w">1.5%</span> <span class="badge-w">2.0%</span>
                            <span class="badge-w">2.5%</span> <span class="badge-w">3.0%</span>
                            — a tighter threshold fires earlier and more often; a wider one waits for a
                            stronger, more decisive move.
                        </p>

                        <h6>Symbol History Mode</h6>
                        <p>
                            Choosing a specific symbol (instead of "All Symbols") switches the scanner
                            from a single day to a <strong style="color:#fff;">date range</strong>: the same
                            day-by-day engine above runs once per trading day in the range and returns one
                            row per day, so you can see how that symbol's breakout signal behaved over time.
                            Ranges are capped at 60 days per request.
                        </p>

                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-sm"
                            style="background:var(--c-lime);color:#000;font-weight:700;font-family:var(--f-display);"
...
@push('script')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            el('mb-hist-start').value = d.toISOString().split('T')[0];
        ...
        /* ── MODE (driven by Symbol select) ── */
        function mbSymbolChanged() {
            var sym = el('mb-sym').value;

            if (sym && sym !== 'ALL') {
                mbMode = 'history';
                el('mb-scan-controls').classList.add('hide');
                el('mb-history-controls').classList.add('show');
                el('mb-scan-btn').style.display = 'none';
                el('mb-hist-btn').style.display = '';
                txt('st-total-label', 'Trading Days');
                txt('mb-card-title-text', sym + ' — Breakout History');
                mbLoadHistory();
            } else {
                mbMode = 'scan';
                el('mb-scan-controls').classList.remove('hide');
                el('mb-history-controls').classList.remove('show');
                el('mb-scan-btn').style.display = '';
                el('mb-hist-btn').style.display = 'none';
                txt('st-total-label', 'Total Records');
                txt('mb-card-title-text', 'Breakout Signals');
                mbScan();
            }
        }

        function mbTriggerCurrent() {
            if (mbMode === 'history') mbLoadHistory();
            else mbScan();
        }
        ...
        function mbSetInst(inst, btn) {
            ...
            if (il) {
                il.textContent = labels[inst];
                il.className = 'mb-inst-label ' + cls[inst];
            }
        ...
        function mbUpdateDateBadge(isToday) {
            var badge = el('mb-date-badge');
            if (!badge) return;
            badge.innerHTML = isToday ?
        ...
        function mbScan() {
            ...
            mbRunScan(params, function(res) {
                if (typeof res.is_today !== 'undefined') mbUpdateDateBadge(res.is_today);
                txt('mb-subtitle', date + ' · ' + (res.message || ''));
            });
        }

        /* ── HISTORY — ONE symbol, DATE RANGE ── */
        function mbLoadHistory() {
            var sym = el('mb-sym').value;
            if (!sym || sym === 'ALL') return;

            var start = el('mb-hist-start').value;
            var end = el('mb-hist-end').value;
            if (!start || !end) return;
            ...
            html('mb-tbody', '<tr><td colspan="9"><div class="mb-spinner-row">' +
                '<div class="mb-spinner"></div>' +
                'Loading ' + sym + ' history ' + start + ' → ' + end + '…' +
                '</div></td></tr>');
            txt('mb-subtitle', start + ' → ' + end + ' · Loading…');
            ...
            mbRunScan(params, function(res) {
                el('mb-date-badge').innerHTML = '<span class="mb-hist-badge">📅 History</span>';
                txt('mb-subtitle', start + ' → ' + end + ' · ' + (res.message || ''));
            });
        }

        /* ── Shared fetch + render for both modes ── */
        function mbRunScan(params, onSuccessExtra) {
        ...
        function mbResetStats() {
            ['st-total', 'st-ce', 'st-pe', 'st-nt'].forEach(function(id) {
                txt(id, '—');
            });
        }
        ...
        function mbHideWarn() {
            el('mb-warn').classList.remove('show');
        }
