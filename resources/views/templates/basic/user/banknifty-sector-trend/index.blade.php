...
<style>
    ...
    /* ── Bias Summary Box ───────────────────────────────── */
    .bias-summary-box { border-radius:12px; padding:18px 22px; margin-bottom:20px; border:1px solid transparent; }
    ...
    .heavy-stock-card.bull { border-left-color:#28a745; }
    ...
    /* ── Trade plan ─────────────────────────────────────── */
    .trade-plan-box { background:#fff; border-radius:10px; padding:16px 20px; margin-bottom:20px; box-shadow:0 2px 8px rgba(0,0,0,0.08); }
...
<div class="container-fluid content-container">
    ...
    <div class="page-header">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
            <div>
                <h4>{{ $pageTitle }} <span class="new-feature-badge">14 STOCKS · WEIGHT-BASED</span></h4>
                <p style="margin:0; font-size:13px;">
                    CE/PE OI analysis for all 14 BANKNIFTY stocks · HDFC+SBI+ICICI = 66% weight · Next day direction
                </p>
            </div>
            <div class="d-flex gap-2 flex-wrap">
                <a href="{{ route('nifty50-sector.index') }}" class="btn btn-light btn-sm"><i class="fas fa-chart-pie"></i> NIFTY50 Trend</a>
    ...
    <div class="alert" style="background:linear-gradient(135deg,#667eea,#764ba2); color:white; border:none; border-radius:12px; margin-bottom:20px; padding:15px;">
        <h6 style="color:white; margin-bottom:10px; font-size:14px;"><i class="fas fa-info-circle"></i> <strong>BANKNIFTY Logic:</strong></h6>
        <div class="row">
            <div class="col-md-3">
                <small style="font-size:11px;"><strong>📊 OI Signal (Same as PE/CE)</strong></small>
            ...
            <div class="col-md-3">
                <small style="font-size:11px;"><strong>👑 Key Weights</strong></small>
                <ul style="font-size:10px; margin-top:5px;">
                    <li>HDFCBANK &nbsp;<strong>25.73%</strong> (top)</li>
                    <li>SBIN &nbsp;<strong>20.66%</strong></li>
                    <li>ICICIBANK &nbsp;<strong>19.71%</strong></li>
                    <li>Top-3 together = <strong>66%</strong> 🔥</li>
                </ul>
            </div>
            <div class="col-md-3">
                <small style="font-size:11px;"><strong>⚖️ Override Rules</strong></small>
                <ul style="font-size:10px; margin-top:5px;">
                    <li>Top-3 aligned → overrides all</li>
                    <li>Top-5 (82%) lean → strong signal</li>
                    <li>10+ of 14 aligned → 92% confidence</li>
                    <li>Weighted bull% vs bear% compared</li>
                </ul>
            </div>
            <div class="col-md-3">
                <small style="font-size:11px;"><strong>⏰ Best Time to Run</strong></small>
                <ul style="font-size:10px; margin-top:5px;">
                    <li>Run at 3:00–3:15 PM EOD</li>
                    <li>Next day entry at 09:30</li>
                    <li>First candle breakout trigger</li>
                    <li>ATM CE / ATM PE option</li>
                </ul>
            </div>
        </div>
    </div>
    ...
    <div class="filter-section">
        <div class="row align-items-end">
            ...
            <div class="col-md-3">
                <label><i class="fas fa-crown"></i> Show:</label>
                <select id="heavy_filter" class="form-control">
                    <option value="">All 14 Stocks</option>
                    <option value="heavy">Top 5 Heavy Only (82%)</option>
                    <option value="top3">Top 3 Only (66%)</option>
                </select>
            </div>
            <div class="col-md-3">
                <label>&nbsp;</label>
                <div>
                    ...
                    <button type="button" id="btn_reset" class="btn btn-outline-light btn-lg ml-2" style="min-width:90px; font-size:13px;">
    ...
    {{-- Stats Row --}}
    <div class="row">
        <div class="col-md-2 col-6"><div class="stats-box"><small>Tracked</small><strong id="stat_total" class="text-dark">0</strong></div></div>
        ...
        <div class="col-md-2 col-6"><div class="stats-box" style="border-left-color:#28a745;"><small>Bull Weight</small><strong id="stat_bull_wt" style="color:#28a745;">0%</strong></div></div>
        <div class="col-md-2 col-6"><div class="stats-box" style="border-left-color:#dc3545;"><small>Bear Weight</small><strong id="stat_bear_wt" style="color:#dc3545;">0%</strong></div></div>
    </div>
    ...
    <div class="bias-summary-box neutral" id="bias_box">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
            <div>
                <div style="font-size:11px; color:#888; text-transform:uppercase; letter-spacing:1px; margin-bottom:8px;">BANKNIFTY Next-Day Bias</div>
            ...
            <div class="text-right">
                ...
                <div id="tp_date_inline"   style="font-size:11px; color:#888; margin-top:2px;">—</div>
            </div>
            <div class="text-right">
                <div style="font-size:11px; color:#888; margin-bottom:4px;">Breadth (14 stocks)</div>
    ...
    {{-- Heavy weights panel --}}
    <div class="heavy-panel" id="heavy_panel" style="display:none;">
        <div class="heavy-panel-header">
            <span style="font-size:20px;">👑</span>
            <h6>Top 5 Heavy Weights — Controls 82% of BANKNIFTY</h6>
            <span class="heavy-tag">KEY MOVERS</span>
        </div>
        <div class="row" id="heavy_cards_row"></div>
        <div class="row mb-2">
            <div class="col-md-4">
    ...
    <div class="trade-plan-box" id="trade_plan_box" style="display:none;">
        <h6><i class="fas fa-bullseye text-danger"></i> Next Day BANKNIFTY Trade Plan</h6>
        <div class="row">
            <div class="col-md-3">
    ...
    <div style="position:relative; min-height:400px;">
        <div class="loading-overlay" id="loading-overlay" style="display:none;">
            ...
            <div class="loading-text">Analyzing 14 BANKNIFTY stocks...</div>
        </div>
                ...
                <tbody id="stock-tbody">
                    <tr>
                        <td colspan="17" class="text-center py-5">
                            <i class="fas fa-university" style="font-size:3rem; opacity:0.4;"></i>
...
const TOP3   = ['HDFCBANK', 'SBIN', 'ICICIBANK'];
const TOP5   = ['HDFCBANK', 'SBIN', 'ICICIBANK', 'AXISBANK', 'KOTAKBANK'];

/* ── Loading ──────────────────────────────────────────────────────────────── */
function toggleLoading(show) {
    if (show) { $('#loading-overlay').show(); $('#btn_analyze').prop('disabled', true); }
    else       { $('#loading-overlay').hide(); $('#btn_analyze').prop('disabled', false); }
}
...
function runAnalysis() {
    ...
    $.ajax({
        url : '{{ route("banknifty-sector.analyze") }}',
        type: 'GET',
        data: { date },
        success: function(res) {
            ...
            allData = res;
            $('#analyzed_at_label').text('Analyzed: ' + res.analyzed_at + ' | Prev: ' + res.prev_date);
            renderAll(res);
            applyFilters();
        },
        error: function() {
...
function renderAll(data) {
    ...
    $('#bias_box').removeClass('bullish bearish neutral').addClass(dirClass);
    ...
    $('#tp_action_inline').html(`<span class="${optClass} font-weight-bold">${plan.action}</span>`);
    $('#tp_strike_inline').text('Strike: ' + plan.strike);
    $('#tp_date_inline').text(plan.trade_date + ' @ ' + plan.entry_time);

    // Trade plan box
    $('#trade_plan_box').show();
    $('#tp_date').text(plan.trade_date);
    const actBadge = plan.option_type === 'CE'
    ...
    // Heavy panel
    renderHeavyPanel(data.heavy_summary, bias);
}

/* ── Heavy Panel ──────────────────────────────────────────────────────────── */
function renderHeavyPanel(heavyStocks, bias) {
    $('#heavy_panel').show();

    $('#top3_bull').text(bias.top3_bull + '%');
    $('#top3_bear').text(bias.top3_bear + '%');
    $('#heavy_bull').text(bias.heavy_bull + '%');
    $('#heavy_bear').text(bias.heavy_bear + '%');
    $('#heavy_confidence').text(bias.confidence + '% (' + bias.strength + ')');

    let html = '';
    heavyStocks.forEach(s => {
        const sc = s.signal === 'BULLISH' ? 'bull' : s.signal === 'BEARISH' ? 'bear' : 'neut';
        ...
        const isTop3 = TOP3.includes(s.symbol);
        const ceCls  = s.ce_oi_pct > 0 ? 'text-success' : 'text-danger';
        const peCls  = s.pe_oi_pct > 0 ? 'text-success' : 'text-danger';

        html += `
        <div class="col-md-2 col-sm-4 col-6">
            <div class="heavy-stock-card ${sc}">
                <div class="d-flex justify-content-between align-items-center">
                    <span class="heavy-stock-sym">${s.symbol}</span>
                    ${isTop3 ? '<span class="heavy-badge">Top-3</span>' : ''}
                </div>
                <div class="heavy-stock-wt">${s.weight}% weight</div>
                <div class="heavy-stock-co">${s.company}</div>
                <div class="mt-2">${sigBadge}</div>
                <div style="font-size:9px; margin-top:6px; color:rgba(255,255,255,0.6);">
                    CE: <span class="${ceCls}">${s.ce_oi_pct > 0 ? '+' : ''}${s.ce_oi_pct}%</span>
                    &nbsp; PE: <span class="${peCls}">${s.pe_oi_pct > 0 ? '+' : ''}${s.pe_oi_pct}%</span>
                </div>
            </div>
        </div>`;
    });
    $('#heavy_cards_row').html(html);
}
...
function applyFilters() {
    ...
    const heavyFilter = $('#heavy_filter').val();

    let stocks = allData.stocks || [];
    if (sigFilter)              stocks = stocks.filter(s => s.signal === sigFilter);
    if (heavyFilter === 'top3') stocks = stocks.filter(s => TOP3.includes(s.symbol));
    if (heavyFilter === 'heavy') stocks = stocks.filter(s => TOP5.includes(s.symbol));
...
function renderTable(stocks) {
    ...
    stocks.forEach((row, i) => {
        ...
        let strongerBadge = '<span class="text-muted">—</span>';
        if (row.stronger_side === 'CE') strongerBadge = '<span class="badge badge-warning" style="font-size:10px;color:#155724;font-weight:700;">CE 💪</span>';
        if (row.stronger_side === 'PE') strongerBadge = '<span class="badge badge-danger" style="font-size:10px;font-weight:700;">PE 💪</span>';

        const isHeavy = row.is_heavy;
        const wt      = row.weight;
        const wtFill  = Math.min(wt, 30) / 30 * 100; // scale to 30% max for visual

        const rowStyle = isHeavy
            ? 'style="background:rgba(245,166,35,0.05); outline:1px solid rgba(245,166,35,0.2);"'
            : '';
        ...
        html += `<tr ${rowStyle}>
            ...
            <td><span class="${condCls}">${row.condition || 'N/A'}</span></td>
            <td>${sentBadge}</td>
            <td>${actBadge}</td>
            <td>${strBadge}</td>
            <td><span class="${ss.cls}">${ss.label}</span></td>
            <td>${strongerBadge}</td>
...
$('#btn_reset').click(function() {
    ...
    $('#signal_filter, #heavy_filter').val('');
    allData = null;
    ...
    $('#heavy_panel, #trade_plan_box').hide();
...
$('#signal_filter, #heavy_filter').change(function() {
