{{-- resources/views/templates/basic/user/options-new/swing.blade.php --}}
...
.overnight-note{background:rgba(245,158,11,.05);border:1px solid rgba(245,158,11,.2);border-radius:8px;padding:9px 14px;margin-bottom:14px;font-size:.74rem;color:#f59e0b}
/* Result badges */
.res-win{color:#3fb950;font-weight:700}
.res-loss{color:#f85149;font-weight:700}
.res-mu{color:#8b949e}
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <div>
        <div class="op-h1">🌙 Swing Signals</div>
        <div class="op-sub">All symbols · Signal 13:30–14:30 · Entry at 14:30 close · Exit next day SL/TGT/15:00 · SL 25% / Target 50%</div>
    </div>
    <div class="d-flex gap-2">
        <a href="{{ route('options.intraday') }}" class="btn-o">← Intraday</a>
        <a href="{{ route('options.swing-backtest') }}" class="btn-o">Backtest</a>
    </div>
</div>
...
<div id="market-banner">
    ...
    <span style="margin-left:auto;font-size:.7rem;opacity:.7">NIFTY direction filter active</span>
</div>

<div class="overnight-note">
    ⚠️ <strong>Overnight positions.</strong>
    Entry at 14:30 candle close · Exit next day: SL hit / Target hit / 15:00 forced close · Check global cues before entering.
    <strong>Changes: SL reduced to 25%, entry moved to 14:30 (avoid EOD peak), market direction filter ON.</strong>
</div>
...
<div class="sig-wrap">
    <div class="sig-th">
        <span style="color:#e2e8f0;font-size:.85rem;font-weight:700">Swing Decision Table</span>
        <small style="color:#8b949e;font-size:.72rem">Entry 14:30 · Exit next day SL/TGT/15:00</small>
    </div>
    <div id="table-container">
        <div class="loader-wrap"><div class="spinner"></div><div>Loading swing signals…</div></div>
    </div>
</div>
...
@push('script')
<script>
const AJAX_URL = '{{ route("options.swing-data") }}';
...
function loadData(date) {
    ...
    fetch(AJAX_URL + (d ? '?date=' + d : ''))
        ...
        .then(data => {
            ...
            ['Symbol','Action','Strike','Entry (14:30)','SL (25%)','Target (50%)','Score','Conf','PCR','OI Zone','IV','Result','Exit','Reason'].forEach(h => {
                html += '<th>' + h + '</th>';
            });
            html += '</tr></thead><tbody>';
            ...
            rows.forEach(r => {
                ...
                if (r.active && r.side) {
                    actionHtml = '<span class="act ' + r.side.toLowerCase() + '">'
                        ...
                        + '<div style="color:#8b949e;font-size:.63rem;margin-top:2px">Entry 14:30</div>';
                } else if (r.blocks && r.blocks.length) {
                    // Show market block specially
                ...
                // Result
                const res = r.result;
                let resultHtml = '<span class="res-mu">—</span>';
                let exitHtml   = '<span class="res-mu">—</span>';
                if (res && res.status !== 'unknown') {
                    const rc  = res.status === 'win' ? 'res-win' : 'res-loss';
                    resultHtml = '<span class="' + rc + '">' + (res.label || '—') + '</span>';
                    ...
                    resultHtml = '<span class="res-mu">' + res.label + '</span>';
                }
                ...
                html += `<tr class="${r.active ? 'go' : 'avoid'}">
                    <td><div class="sym">${r.symbol}</div><div style="color:#8b949e;font-size:.65rem">Signal: ${r.time}</div></td>
                    <td>${actionHtml}</td>
                    ...
                    <td>${r.premium ? '₹' + r.premium : '—'}<div style="color:#8b949e;font-size:.62rem">14:30 close</div></td>
                    <td class="c-dn">${r.sl ? '₹' + r.sl : '—'}</td>
                    <td class="c-up">${r.tgt ? '₹' + r.tgt : '—'}</td>
                    <td><span style="font-weight:800;font-size:.9rem;color:${scColor}">${sc > 0 ? '+' : ''}${sc}</span></td>
                    <td>
                        <div class="cf">
