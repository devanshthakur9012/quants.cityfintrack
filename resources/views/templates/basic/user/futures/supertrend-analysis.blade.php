    ...
    <div class="container-fluid content-container">
        ...
        <div class="mb-4">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h4>{{ $pageTitle }}</h4>
                    <p class="text-muted">Complete technical analysis with Supertrend, VWAP, RSI & MACD</p>
                </div>
                <div>
                    <a href="{{ route('futures.config') }}" class="btn btn-warning me-2">
                        <i class="fas fa-cog"></i> Configure Indicators
                    </a>
...
<script>
    ...
    function loadSupertrendData() {
        ...
        $.ajax({
            url: '{{ route("futures.supertrend-fetch") }}',
            type: 'GET',
            data: {
    ...
    //         // VWAP styling
    //         const vwapSignalClass = row.vwap_signal === 'BUY' ? 'badge-buy' : 
    //                                 row.vwap_signal === 'SELL' ? 'badge-sell' : 'badge-hold';
    //         const vwapSignalIcon = row.vwap_signal === 'BUY' ? '▲' : 
    //                                row.vwap_signal === 'SELL' ? '▼' : '→';
    ...
    //                 <td><span class="badge ${vwapSignalClass}"><span class="signal-icon">${vwapSignalIcon}</span> ${row.vwap_signal || 'HOLD'}</span></td>
    ...
    function displayTable() {
        ...
        currentData.forEach(function (row) {
            ...
            // The backend already converted GAP_UP → BUY and GAP_DOWN → SELL
            // So we just need to display it directly
    ...
    function updateStatistics(data) {
        ...
        const latestVWAP = (latestRecord.vwap != null && !isNaN(latestRecord.vwap)) ? parseFloat(latestRecord.vwap).toFixed(2) : '-';
        const latestRSI = (latestRecord.rsi != null && !isNaN(latestRecord.rsi)) ? parseFloat(latestRecord.rsi).toFixed(2) : '-';
    ...
    function exportToCSV() {
        ...
        window.location.href = '{{ route("futures.export") }}?' + params.toString();
    }
    ...
    $(document).ready(function () {
        ...
        $('#manual_fetch_btn').click(function () {
            if (!confirm('This will fetch the latest data from Zerodha for all monitored futures. Continue?')) {
                return;
            }
            ...
            const $status = $('<div class="alert alert-info fetch-status">Fetching latest data from Zerodha...</div>');
            $('body').append($status);

            $.ajax({
                url: '{{ route("futures.manual-fetch") }}',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
