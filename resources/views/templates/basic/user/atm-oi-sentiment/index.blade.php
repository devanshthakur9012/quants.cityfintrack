...
.custom--table thead th,
.custom--table tbody td { text-align:center !important; padding:8px 6px !important; font-size:11px !important; vertical-align:middle; }
...
.atm-badge { background:rgba(249,202,36,0.15); color:#f9ca24; border:1px solid rgba(249,202,36,0.3); border-radius:5px; padding:2px 7px; font-size:9px; font-weight:700; display:inline-block; }
...
<div class="container-fluid content-container">
    ...
    <div class="page-header">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
            <div>
                <h4>{{ $pageTitle }}
                    <span class="new-feature-badge">ATM Only</span>
                </h4>
                <p style="color:rgba(255,255,255,0.85);margin:4px 0 0;font-size:12px;">
                    Single ATM strike &nbsp;&middot;&nbsp; strike_position = ATM &nbsp;&middot;&nbsp;
                    <span style="color:#51cf66;">&#9679; Original</span> &nbsp;
                    <span style="color:#a29bfe;">&#9679; Pivot Daily (prev vs day-before)</span> &nbsp;
                    <span style="color:#f9ca24;">&#9679; New Logic + Strength</span>
                </p>
            </div>
            <div>
                <a href="{{ route('daily-oi-sentiment.index') }}" class="btn btn-light btn-sm mr-2">
                    <i class="fas fa-layer-group"></i> All Strikes View
                </a>
                <a href="{{ route('oiiv-auto.pece-analysis') }}" class="btn btn-light btn-sm">
        ...
        <div class="row">
            <div class="col-md-4">
                ...
                <ul style="font-size:10px;color:rgba(255,255,255,.7);margin-top:4px;">
                    <li>Source: OptionOhlcData ATM 14:45 vs prev 15:15</li>
                    <li>CE ↑ + PE ↓ → BEARISH &nbsp; CE ↓ + PE ↑ → BULLISH</li>
                    <li>Both ↑ → CE%>PE% = BEARISH &nbsp; Both ↓ → CE%&lt;PE% = BULLISH</li>
                </ul>
            </div>
            <div class="col-md-4">
                <small style="color:#a29bfe;font-weight:700;font-size:11px;">&#128197; Col 2 — Pivot Daily</small>
                <ul style="font-size:10px;color:rgba(255,255,255,.7);margin-top:4px;">
                    <li>Source: OptionDailyOhlcData, same ATM strike</li>
                    <li>prevDay OI vs dayBeforePrev OI</li>
            ...
            <div class="col-md-4">
                ...
                <ul style="font-size:10px;color:rgba(255,255,255,.7);margin-top:4px;">
                    <li>Same ATM CE%/PE% as Col 1</li>
                    <li>CE ↑+PE ↓ → BEARISH &nbsp; CE ↓+PE ↑ → BULLISH</li>
                    <li>Both ↑ → PE%>CE% = BULLISH &nbsp; Both ↓ → |CE%|>|PE%| = BULLISH</li>
                </ul>
            </div>
        </div>
    </div>
    ...
    <div class="row mb-3">
        <div class="col-6 col-md-2">
            <div class="stats-box"><small>Total Records</small><strong id="st-total" class="text-dark">0</strong></div>
        </div>
        <div class="col-6 col-md-2">
            <div class="stats-box" style="border-left-color:#28a745;">
                <small>&#10003; Orig Bullish</small><strong id="st-orig-bull" style="color:#28a745;">0</strong>
            </div>
        </div>
        <div class="col-6 col-md-2">
            <div class="stats-box" style="border-left-color:#dc3545;">
                <small>&#10003; Orig Bearish</small><strong id="st-orig-bear" style="color:#dc3545;">0</strong>
            </div>
        </div>
        <div class="col-6 col-md-2">
            <div class="stats-box" style="border-left-color:#a29bfe;">
                <small>&#128197; Pivot Bullish</small><strong id="st-piv-bull" style="color:#a29bfe;">0</strong>
            </div>
        </div>
        <div class="col-6 col-md-2">
            <div class="stats-box" style="border-left-color:#f9ca24;">
                <small>&#9889; New Bullish</small><strong id="st-new-bull" style="color:#e2b900;">0</strong>
            </div>
        </div>
        <div class="col-6 col-md-2">
            <div class="stats-box" style="border-left-color:#fd7e14;">
                <small>&#9889; New Bearish</small><strong id="st-new-bear" style="color:#fd7e14;">0</strong>
            </div>
        </div>
    </div>
    ...
    <div style="position:relative;min-height:400px;">
        <div class="loading-overlay" id="loading-overlay" style="display:none;">
            ...
            <div class="loading-text">Loading ATM data...</div>
        </div>
            ...
            <table class="table custom--table">
                <thead>
                    <tr>
                        ...
                        {{-- Raw --}}
                        <th colspan="5" class="th-raw">&#128202; ATM OI Data</th>

                        {{-- Col 1 — Original --}}
                        <th colspan="2" class="th-orig">&#10003; Original Signal<br><small style="font-weight:400;opacity:.75;">unchanged logic</small></th>

                        {{-- Col 2 — Pivot Daily --}}
                        <th colspan="3" class="th-pivot">&#128197; Pivot Daily<br><small style="font-weight:400;opacity:.75;">OptionDailyOhlcData · ATM strike · prev vs day-before</small></th>

                        {{-- Col 3 — New Logic --}}
                        <th colspan="3" class="th-new">&#9889; New Logic + Strength<br><small style="font-weight:400;opacity:.75;">corrected 4-case on same ATM CE/PE%</small></th>
                    </tr>
                    <tr>
                        {{-- Raw --}}
                        <th class="th-raw">ATM Strike</th>
                ...
                <tbody id="sig-tbody">
                    <tr>
                        <td colspan="16" class="text-center py-5">
                            <i class="fas fa-crosshairs" style="font-size:3rem;opacity:.4;"></i>
                            <p style="margin-top:18px;">Click <strong>"View Data"</strong> to load ATM signals</p>
                        </td>
                    </tr>
                </tbody>
            </table>
...
function loadSymbols() {
    $.get('{{ route("atm-oi-sentiment.symbols") }}', function (res) {
...
function runAnalysis() {
    ...
    tableData = [];
    $.ajax({
        url : '{{ route("atm-oi-sentiment.analyze") }}',
        type: 'GET',
        ...
        success(res) {
            ...
            if (res.success && res.data && res.data.length > 0) {
                ...
                showNoData(res.message || 'No ATM data found. Check if strike_position = ATM exists for this date.');
                resetStats();
            }
        },
        error(xhr) {
            toggleLoading(false);
            showNoData('&#9888; ' + (xhr.responseJSON?.message || 'Server error'));
            resetStats();
        }
    });
}
...
function strBadge(strength, diff) {
    if (!strength) return '<span class="sig-na">&mdash;</span>';
...
function renderTable() {
    ...
    tableData.forEach(function (d, i) {
        const ceCls = d.ce_oi_pct > 0 ? 'text-success' : d.ce_oi_pct < 0 ? 'text-danger' : '';
        const peCls = d.pe_oi_pct > 0 ? 'text-success' : d.pe_oi_pct < 0 ? 'text-danger' : '';
        ...
        <tr>
            ...
            <td><span class="atm-badge">&#8377;${Number(d.atm_strike || d.ce_strike || 0).toLocaleString('en-IN')}</span></td>
            <td class="${ceCls}"><strong>${(d.ce_oi_pct > 0 ? '+' : '') + Number(d.ce_oi_pct).toFixed(2)}%</strong></td>
            <td class="${peCls}"><strong>${(d.pe_oi_pct > 0 ? '+' : '') + Number(d.pe_oi_pct).toFixed(2)}%</strong></td>
...
$('#btn_reset').click(function () {
    ...
    $('#symbol_filter').val('');
    tableData = [];
    showNoData('Click "View Data" to load ATM signals');
    resetStats();
