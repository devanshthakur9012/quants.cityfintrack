...
<style>
    ...
    .spinner { width:55px;height:55px;border:5px solid #f3f3f3;border-top:5px solid #e74c3c;border-radius:50%;animation:spin 1s linear infinite; }
    ...
    /* ── Red/contrarian theme ── */
    .page-header {
        background: linear-gradient(135deg,#4a0000,#7f0000,#c0392b);
        color:white;padding:22px;border-radius:12px;
        margin-bottom:20px;box-shadow:0 4px 20px rgba(192,57,43,0.4);
    }
    .filter-section {
    ...
    .rank-n { background:linear-gradient(135deg,#c0392b,#922b21);font-size:10px; }

    .symbol-name { color:#7f0000;font-weight:800;font-size:13px; }
    ...
    .badge-days  { background:#fde8e8;color:#922b21;padding:2px 8px;border-radius:4px;font-weight:700;font-size:11px; }
    ...
    .table-card-header { background:linear-gradient(135deg,#4a0000,#7f0000);color:white;padding:14px 20px;font-weight:700;font-size:14px;display:flex;align-items:center;gap:10px; }
    ...
    .reverse-badge     { background:rgba(255,255,255,0.2);color:white;padding:2px 8px;border-radius:10px;font-size:10px;font-weight:700;letter-spacing:.3px; }
    ...
    .logic-strip {
        background:linear-gradient(135deg,#2c0000,#3d0000);
        color:#fca5a5;font-size:11px;padding:10px 20px;
        display:flex;gap:20px;flex-wrap:wrap;align-items:center;
        border-bottom:1px solid rgba(255,255,255,0.05);
    }
    .logic-strip span { color:#fecaca;font-weight:700; }
...
<div class="container-fluid content-container">
    ...
    <div class="page-header">
        <div class="d-flex justify-content-between align-items-center flex-wrap" style="gap:10px;">
            <div>
                <h4 style="margin:0;">
                    🔄 Universal BTST — <span style="color:#fca5a5;">Reverse / Contrarian</span>
                    <span class="new-feature-badge">100+ Symbols</span>
                    <span class="reverse-badge ml-1">↔ SIGNAL FLIPPED</span>
                </h4>
                <p style="margin:4px 0 0;font-size:11px;opacity:.8;line-height:1.9;">
                    OI says <strong>BULLISH</strong> → We <strong style="color:#fca5a5;">BUY PE</strong> &nbsp;|&nbsp;
                    OI says <strong>BEARISH</strong> → We <strong style="color:#86efac;">BUY CE</strong> &nbsp;|&nbsp;
                    📥 Entry: Blended 15:00 + 15:15 OHLC avg &nbsp;|&nbsp;
                    📤 Exit: Blended 09:20 + 09:25 + 09:30 OHLC avg &nbsp;|&nbsp;
                    Win threshold: &gt;0.5% &nbsp;|&nbsp; Min buy: ₹20
                </p>
            </div>
            <div style="display:flex;gap:8px;flex-wrap:wrap;">
                <a href="{{ route('universal-btst.index') }}"  class="compare-btn">📊 Normal Signal</a>
                <a href="{{ route('account-wise.zzl') }}"      class="compare-btn"><i class="fas fa-wallet"></i> Account-Wise</a>
    ...
    <div class="filter-section">
        <div class="row align-items-end">
            ...
            <div class="col-md-6 col-sm-12 text-center mt-3 mt-md-0" style="padding-top:22px;">
                <button type="button" id="ub_run" class="btn btn-light btn-lg" style="min-width:200px;font-size:13px;font-weight:700;">
                    <i class="fas fa-sync-alt"></i> Scan Reverse Signals
                </button>
    ...
    <div class="row mb-3" id="summary_stats" style="display:none;">
        ...
        <div class="col-6 col-md-2">
            <div class="stats-box" style="border-left-color:#c0392b;">
                <small>Trading Days</small>
                <strong id="st_days" style="color:#c0392b;">0</strong>
            </div>
        </div>
        <div class="col-6 col-md-2">
    ...
    <div class="table-card">
        <div class="table-card-header">
            <span>🔄 Reverse Signal Performance</span>
            ...
            <div style="margin-left:auto;">
                <input type="text" id="symbol_search" class="form-control form-control-sm"
                       ...
                       style="display:none;background:rgba(255,255,255,0.15);border:1px solid rgba(255,255,255,0.3);color:white;">
            </div>
        </div>

        <div class="logic-strip">
            🔄 <span>REVERSE MODE</span>:
            BULLISH OI signal → <span>BUY PE</span> &nbsp;|&nbsp;
            BEARISH OI signal → <span>BUY CE</span> &nbsp;|&nbsp;
        ...
        <div class="loading-overlay" id="ub_loading" style="display:none;">
            ...
            <div class="loading-text">Scanning reverse signals…</div>
                ...
                <tbody id="ub_tbody">
                    <tr>
                        <td colspan="7" class="text-center py-5">
                            <i class="fas fa-sync-alt" style="font-size:3rem;opacity:0.3;color:#c0392b;"></i>
                            <p style="font-size:1.1rem;margin-top:20px;color:#999;">
                                Select a date range and click <strong>"Scan Reverse Signals"</strong>
                            </p>
                        </td>
                    </tr>
                </tbody>
...
$(function () {
    ...
    function buildRow(row, i) {
        ...
        <tr data-symbol="${row.symbol.toLowerCase()}">
            ...
            <td>
                <span class="${winClass(pct)}">${pct.toFixed(1)}%</span>
                <span class="win-bar-wrap">
                    <span class="win-bar-fill" style="width:${barW}%;background:${barColor(pct)};"></span>
                </span>
            </td>
        </tr>`;
    }
    ...
    function showStats(data, meta) {
        ...
        $('#st_noise').text(data.reduce((s, r) => s + (Number(r.noise) || 0), 0));
    ...
    function runScan() {
        ...
        $.ajax({
            url    : '{{ route("universal-btst-reverse.analyze") }}',
            type   : 'GET',
