...
<script>
    $(document).ready(function(){
        $(document).on('click','.buyModal',function(){
            ...
            var quantiy = $(this).data('quantiy');
            var id = $(this).data('id');
            var buyhtml = 
            `<div class="modal-header">
                <h5 class="modal-title" id="buyLabel">${symbol} - ${type} Order</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form action="{{route('user.place-trade-order')}}" class="transparent-form" method="POST">
                    @csrf
                    <div class="mb-0">
                        ...
                        <input type="radio" name="order_type" value="PROFIT_PERCENTAGE" onclick="limitPrice()" id="profit_percentage">
                        <label for="profit_percentage" class="col-form-label">Profit Percentage </label>
                    </div>
                    <div class="mb-3">
                        <label for="price" class="col-form-label">Price:</label>
                        <input type="number" min="0" step="any" name="price" data-price="${ltp}" class="form--control" value="${price}" id="price" placeholder="Enter Your Price" required readonly>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="quantity" class="form--control" value="${quantiy}" min="1" max="${quantiy}" id="quantity" required>
                    </div>
    ...
    function limitPrice(){
        ...
        else if($('#profit_percentage').prop('checked')){
            $('#price').removeAttr("readonly");
        }else{
...
<script>
    ...
    function refreshPortfolioData(dateFilter = null, showLoading = true) {
        ...
        $.ajax({
            url: '{{ route("user.trade-position-master-fetch") }}',
            type: 'GET',
            data: ajaxData,
            ...
            response.positions.forEach(function (data) {
                ...
                tbodyHtml += `
                    <tr>
                        ...
                        <td><span class="badge ${data.txn_type === 'BUY' ? 'bg-primary' : 'bg-danger'}">${data.txn_type}</span></td>
                        <td>${data.lot_size}</td>
                        ...
                        <td><button class="py-0 buyModal ms-1 btn btn-${data.txn_type === "BUY" ? 'danger' : 'success'} btn-sm" data-token="${data.token}" data-id="${data.id}" data-quantiy="${data.buy_quantity}" data-symbol="${data.symbol_name}" data-ltp="${data.latest_ltp}" data-price="${data.latest_ltp}" data-type="${data.txn_type === "BUY" ? 'SELL' : 'BUY'}" data-bs-toggle="modal" data-bs-target="#buy">${data.txn_type === "BUY" ? 'SELL' : 'BUY'}</button></td>
                    </tr>
                `;
            });
        }
