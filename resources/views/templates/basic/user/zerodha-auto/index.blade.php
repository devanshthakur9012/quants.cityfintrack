    ...
    <div class="container content-container">
        ...
        <div class="custom--card">
            <div class="card-header">
                <h5 class="card-title mb-0">Zerodha Auto Trading Configurations</h5>
                <p class="text-muted small">Automatically trade CE/PE options based on signal strategy</p>
            </div>
            <div class="card-body p-0">
                {{-- <div class="alert alert--info m-3">
                    <i class="las la-info-circle"></i>
                    <strong>Signal Strategies (1-Minute Interval):</strong>
                    <ul class="mb-0 mt-2">
                        <li><strong>Supertrend Only:</strong> Place orders when Supertrend signals BUY/SELL</li>
                        <li><strong>VWAP Only:</strong> Place orders when VWAP signals BUY/SELL (GAP_UP/GAP_DOWN)</li>
                        <li><strong>Both:</strong> Place orders only when BOTH indicators synchronize</li>
                    </ul>
                    <div class="mt-2">
                        <strong>Option Type Filter:</strong>
                        <ul class="mb-0 mt-1">
                            <li><strong>CE Only:</strong> Trade only Call options (BUY signals only)</li>
                            <li><strong>PE Only:</strong> Trade only Put options (SELL signals only)</li>
                            <li><strong>BOTH:</strong> Trade both CE and PE based on signals</li>
                        </ul>
                    </div>
                    <div class="mt-2">
                        <strong>Quality Momentum Filter (Recommended):</strong>
                        <ul class="mb-0 mt-1">
                            <li>Checks: 4/6 recent candles with volume > SMA(15)</li>
                            <li>Checks: 4/5 recent closes trending in signal direction</li>
                            <li>Result: Filters weak/choppy signals, improves win rate</li>
                        </ul>
                    </div>
                    <div class="mt-2">
                        <strong>Freeze Quantity Handling:</strong>
                        <ul class="mb-0 mt-1">
                            <li>NIFTY: 18 lots max per order</li>
                            <li>BANKNIFTY: 20 lots max per order</li>
                            <li>Orders exceeding limits are automatically split</li>
                        </ul>
                    </div>
                </div> --}}
                        ...
                        <tbody>
                            @forelse ($configs as $config)
                                <tr>
                                    ...
                                    <!-- ✅ ADDED -->
                                    <td>
                                        <span class="badge badge--{{ $config->option_filter == 'BOTH' ? 'primary' : ($config->option_filter == 'CE' ? 'success' : 'danger') }}" 
                                              title="{{ $config->option_filter == 'CE' ? 'Call Options Only' : ($config->option_filter == 'PE' ? 'Put Options Only' : 'Both CE & PE') }}">
                                    ...
                                    {{-- <td>
                                        @if ($config->enable_quality_filter)
                                            <span class="badge badge--success" title="Quality filter enabled">
                                                <i class="las la-check-circle"></i> ON
                                            </span>
                                        @else
                                            <span class="badge badge--secondary" title="Quality filter disabled">
                                                <i class="las la-times-circle"></i> OFF
                                            </span>
                                        @endif
                                    </td> --}}
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            <a href="{{ route('user.zerodha-auto.orders', $config->id) }}" 
...
<div class="modal fade" id="configModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form id="configForm" action="{{ route('user.zerodha-auto.store') }}" method="POST">
                @csrf
                ...
                <div class="modal-body">
                    <div class="alert alert--warning">
                        ...
                        <strong>Important:</strong> This config applies to ALL monitored futures on 1-minute interval. 
                        Choose your signal strategy carefully.
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-6 form-group">
                            ...
                            <select name="signal_strategy" class="form--control" required>
                                <option value="BOTH" selected>BOTH (Supertrend + VWAP)</option>
                                <option value="SUPERTREND">Supertrend Only</option>
                                <option value="VWAP">VWAP Only</option>
                            </select>
                            <small class="text-muted">Choose when to place orders (1-minute data)</small>
                        </div>
                        ...
                        <!-- ✅ ADDED: Option Filter Dropdown -->
                        <div class="col-lg-6 form-group">
                            <label class="required">Option Type Filter<sup class="text--danger">*</sup></label>
                            <select name="option_filter" class="form--control" required>
                                <option value="BOTH" selected>BOTH (CE & PE)</option>
                                <option value="CE">CE Only (Call Options)</option>
                                <option value="PE">PE Only (Put Options)</option>
                            </select>
                            <small class="text-muted">
                                <strong>CE:</strong> Trade only BUY signals (Call) | 
                                <strong>PE:</strong> Trade only SELL signals (Put) |
                                <strong>BOTH:</strong> Trade all signals
                            </small>
                        </div>
                        ...
                        <div class="col-lg-6 form-group">
                            ...
                            <input type="number" name="profit_percent" class="form--control" 
                                ...
                                value="5.00" required>
                            <small class="text-muted">Auto-SELL when executed orders reach this profit %</small>
                        </div>

                        {{-- <div class="col-lg-6 form-group">
                            <label>Quality Momentum Filter</label>
                            <div class="form-check form-switch mt-2">
                                <input class="form-check-input" type="checkbox" name="enable_quality_filter" 
                                    id="enable_quality_filter" value="1" checked>
                                <label class="form-check-label" for="enable_quality_filter">
                                    Enable Quality Filter (Recommended)
                                </label>
                            </div>
                            <small class="text-muted">
                                Filters weak signals by checking volume surge & price momentum consistency
                            </small>
                        </div> --}}
                        ...
                        <div class="col-lg-6 form-group">
                            ...
                            <small class="text-muted">For NIFTY, BANKNIFTY, FINNIFTY (auto-splits if > freeze limit)</small>
                        </div>
...
$(document).ready(function() {
    $('.toggle-status').click(function() {
        ...
        const url = "{{ route('user.zerodha-auto.toggle', ':id') }}".replace(':id', configId);
    ...
    $('.delete-config').click(function() {
        const configId = $(this).data('id');
        const url = "{{ route('user.zerodha-auto.destroy', ':id') }}".replace(':id', configId);
    ...
    $('.edit-config').on('click', function () {
        ...
        $('#configForm').attr(
            'action',
            "{{ route('user.zerodha-auto.update', ':id') }}".replace(':id', config.id)
        );
        ...
        $('[name="signal_strategy"]').val(config.signal_strategy);
        $('[name="option_series"]').val(config.option_series);
        $('[name="option_filter"]').val(config.option_filter);
