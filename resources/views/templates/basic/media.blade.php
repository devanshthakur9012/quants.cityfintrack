...
.qmd-gallery-panel.active {
    display: block;
    animation: mdFadeUp .4s ease both;
}
...
.qmd-lb-close {
    ...
    color: #333; box-shadow: 0 4px 16px rgba(0,0,0,.4);
...
<div class="qmd-content">
    ...
    <div class="qmd-gallery-panel {{ $idx === 0 ? 'active' : '' }}"
        ...
        {{-- Load More Button (only when total > 20) --}}
...
function switchCatTab(catId, btn) {
    document.querySelectorAll('.qmd-cat-tab').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
    document.querySelectorAll('.qmd-gallery-panel').forEach(p => p.classList.remove('active'));
    const panel = document.getElementById('catPanel' + catId);
    if (panel) panel.classList.add('active');
    // reset lightbox items for new tab
    lbItems   = [];
    lbCurrent = 0;
}
...
function loadMore(catId, btn) {
    ...
    const wrap   = document.getElementById('loadmorewrap_' + catId);
    const offset = parseInt(btn.dataset.offset);
    const total  = parseInt(panel.dataset.total);
    const url    = panel.dataset.loadmoreUrl + '?offset=' + offset;
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            if (remaining <= 0) {
                ...
                const lbl = btn.querySelector('.qmd-remaining');
                if (lbl) lbl.textContent = '(' + remaining + ' more)';
            }
        })
        .catch(() => alert('Failed to load more items. Please try again.'))
...
function openLightbox(el, catId) {
    // Build items list from current active grid
    const grid = document.getElementById('grid_' + catId);
    if (!grid) return;

    lbItems = Array.from(grid.querySelectorAll('.qmd-gallery-item')).map(card => ({
    ...
    // Find clicked index
    const allCards = Array.from(grid.querySelectorAll('.qmd-gallery-item'));
