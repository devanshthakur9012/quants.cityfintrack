...
.qa-wrap {
    ...
    color: #E4EBF5; display: block; background: #06101A;
}
...
@keyframes qaFadeUp { from{opacity:0;transform:translateY(32px)} to{opacity:1;transform:none} }
.qa-anim { animation: qaFadeUp .7s ease both; }
.qa-anim.d1{animation-delay:.1s} .qa-anim.d2{animation-delay:.2s}
.qa-anim.d3{animation-delay:.3s} .qa-anim.d4{animation-delay:.4s}

.qa-sec { padding: 80px 24px; display: block; }
.qa-title {
    display:block; text-align:center;
    font-family:'Rajdhani',sans-serif; font-size:clamp(26px,3.2vw,38px);
    font-weight:700; color:#7DFF00; margin-bottom:10px;
}
.qa-line {
    display:block; width:60px; height:3px; margin:0 auto 14px; border-radius:2px;
    background:linear-gradient(90deg,rgba(255,255,255,.6),#7DFF00);
}
.qa-sub {
    display:block; text-align:center; color:#7A90B5; font-size:15px;
    max-width:600px; margin:0 auto 52px; line-height:1.8;
}
...
.qa-hero::before {
    ...
    animation:gridScroll 18s linear infinite; pointer-events:none;
}
@keyframes gridScroll { from{background-position:0 0} to{background-position:64px 64px} }
.qa-hero::after {
    content:''; position:absolute; inset:0;
...
.qa-hero-inner { position:relative; z-index:1; text-align:center; padding:60px 24px; }
.qa-hero-badge {
    display:inline-flex; align-items:center; gap:8px;
    background:rgba(245,166,35,.12); border:1px solid rgba(245,166,35,.3);
    color:#7DFF00; font-size:12px; font-weight:700;
    padding:7px 18px; border-radius:30px; letter-spacing:.1em;
    text-transform:uppercase; margin-bottom:22px;
}
.qa-hero-h1 {
    font-family:'Rajdhani',sans-serif; font-size:clamp(38px,6vw,70px);
    font-weight:700; color:#fff; line-height:1.05; margin-bottom:20px;
}
.qa-hero-h1 span { color:#7DFF00; }
.qa-hero-p {
...
.qa-hero-stats {
    ...
    background:rgba(0,0,0,.25); backdrop-filter:blur(10px); flex-wrap:wrap;
}
.qa-hstat {
...
.qa-hstat-val {
    font-family:'Rajdhani',sans-serif; font-size:clamp(22px,2.5vw,30px);
    font-weight:700; color:#7DFF00; display:block; line-height:1; margin-bottom:4px;
}
.qa-hstat-lbl { font-size:11px; color:#7A90B5; letter-spacing:.06em; text-transform:uppercase; }
@media(max-width:600px){ .qa-hstat{padding:14px 20px} }

/* ── WHO WE ARE + MISSION ─────────────────────────────── */
.qa-wm-sec { background:#091828; padding:80px 24px; }
.qa-wm-inner {
...
.qa-wm-card {
    ...
    border:1px solid rgba(255,255,255,.07); border-radius:22px; padding:44px 38px;
    position:relative; overflow:hidden; transition:border-color .3s,box-shadow .3s;
}
.qa-wm-card:hover { border-color:rgba(245,166,35,.25); box-shadow:0 0 40px rgba(245,166,35,.1); }
.qa-wm-card::before {
    content:''; position:absolute; top:-1px; left:24px; right:24px; height:2px;
    background:linear-gradient(90deg,transparent,#7DFF00,transparent); border-radius:2px;
}
.qa-wm-icon {
    width:52px; height:52px; border-radius:14px;
    background:rgba(245,166,35,.12); border:1px solid rgba(245,166,35,.25);
    display:flex; align-items:center; justify-content:center;
    font-size:22px; color:#7DFF00; margin-bottom:22px;
}
.qa-wm-card h3 { font-size:clamp(22px,2.5vw,28px); font-weight:700; color:#fff; margin-bottom:16px; }
...
.qa-pillar i { color:#7DFF00; font-size:13px; }
...
.qa-value {
    ...
    border-radius:10px; padding:14px 16px; transition:background .25s,border-color .25s;
}
...
.qa-value-icon {
    ...
    justify-content:center; font-size:14px; color:#7DFF00;
}
...
.qa-ideator-card {
    ...
    border:1px solid rgba(255,255,255,.07); border-radius:22px; overflow:hidden;
...
.qa-id-av {
    width:96px; height:96px; border-radius:50%; border:3px solid #7DFF00;
...
.qa-id-role  { font-size:13px; color:#7DFF00; font-weight:600; margin-bottom:6px; }
.qa-id-creds {
...
.qa-id-social {
    ...
    color:#7A90B5; font-size:13px; display:flex; align-items:center;
    justify-content:center; transition:all .22s;
}
.qa-id-social:hover { background:rgba(245,166,35,.15); border-color:#7DFF00; color:#7DFF00; }

/* ── WORKSPACE ────────────────────────────────────────── */
.qa-workspace-sec { background:#091828; padding:80px 24px; }
.qa-ws-slider-wrap {
    max-width:1100px; margin:0 auto 52px; position:relative;
    border-radius:20px; overflow:hidden;
    border:1px solid rgba(245,166,35,.18);
    box-shadow:0 24px 72px rgba(0,0,0,.55),0 0 0 1px rgba(245,166,35,.08);
}
.qa-ws-track { display:flex; transition:transform .6s cubic-bezier(.4,0,.2,1); }
.qa-ws-slide {
    min-width:100%; position:relative; aspect-ratio:16/7;
    background:linear-gradient(135deg,#0E1E35,#142540); flex-shrink:0;
}
.qa-ws-slide img { width:100%; height:100%; object-fit:cover; display:block; }
.qa-ws-slide-overlay {
...
.qa-ws-caption-badge {
    ...
    color:#7DFF00; font-size:11px; font-weight:700;
...
.qa-ws-arrow {
    ...
    color:#7DFF00; font-size:15px; cursor:pointer;
    display:flex; align-items:center; justify-content:center;
    transition:all .25s; backdrop-filter:blur(8px);
}
.qa-ws-arrow:hover { background:#7DFF00; color:#000; border-color:#7DFF00; }
...
.qa-ws-dot.on { background:#7DFF00; width:22px; border-radius:4px; }
.qa-ws-counter {
    ...
    padding:5px 13px; border-radius:20px; backdrop-filter:blur(8px); letter-spacing:.05em;
}

/* city tabs */
.qa-city-wrap { max-width:1100px; margin:0 auto; }
.qa-city-tabs { display:flex; gap:10px; flex-wrap:wrap; justify-content:center; margin-bottom:28px; }
.qa-city-tab {
    display:inline-flex; align-items:center; gap:8px;
...
.qa-city-tab.on {
    ...
    color:#7DFF00; box-shadow:0 0 18px rgba(245,166,35,.15);
}
...
.qa-city-card {
    background:linear-gradient(135deg,#0E1E35,#142540);
    border:1px solid rgba(245,166,35,.18); border-radius:20px; overflow:hidden;
...
.qa-city-photo {
    ...
    background:linear-gradient(135deg,#081220,#1a2f50); overflow:hidden;
}
...
.qa-city-flag { font-size:32px; margin-bottom:14px; line-height:1; }
.qa-city-name {
    font-family:'Rajdhani',sans-serif; font-size:clamp(24px,2.8vw,32px);
    font-weight:700; color:#fff; margin-bottom:6px;
}
.qa-city-tag {
    display:inline-block; background:#7DFF00; color:#000;
...
.qa-city-meta-row { display:flex; align-items:flex-start; gap:10px; font-size:13px; color:#B0C0D8; }
.qa-city-meta-row i { color:#7DFF00; font-size:12px; margin-top:2px; flex-shrink:0; }
...
.qa-ceo-sec::before {
    ...
    font-family:'Rajdhani',sans-serif; font-size:320px;
    color:rgba(245,166,35,.03); line-height:1; pointer-events:none; user-select:none;
}
.qa-ceo-inner {
...
.qa-ceo-card {
    background:linear-gradient(160deg,#0E1E35,#142540);
    border:1px solid rgba(245,166,35,.22); border-radius:22px;
    padding:40px 28px; text-align:center; position:sticky; top:90px;
    box-shadow:0 0 40px rgba(245,166,35,.1);
}
.qa-ceo-av {
    width:120px; height:120px; border-radius:50%; border:4px solid #7DFF00;
    background:linear-gradient(135deg,#1a3a6e,#d4840e);
    display:flex; align-items:center; justify-content:center;
...
.qa-ceo-name  { font-family:'Rajdhani',sans-serif; font-size:24px; font-weight:700; color:#fff; margin-bottom:6px; }
.qa-ceo-title { font-size:13px; color:#7DFF00; font-weight:600; margin-bottom:20px; }
.qa-ceo-socials { display:flex; gap:10px; justify-content:center; margin-top:14px; }
.qa-ceo-social {
...
.qa-ceo-social:hover { background:rgba(245,166,35,.15); border-color:#7DFF00; color:#7DFF00; }
.qa-ceo-sig {
    font-family:'Exo 2',sans-serif; font-style:italic; font-size:22px;
    color:#7DFF00; margin-top:22px;
...
.qa-vision-h span { color:#7DFF00; }
.qa-vision-body p {
    color:#7A90B5; font-size:15px; line-height:1.92; margin-bottom:18px;
    padding-left:18px; border-left:2px solid rgba(245,166,35,.2); transition:border-color .3s;
}
.qa-vision-body p:hover { border-left-color:#7DFF00; color:#B0C0D8; }
...
.qa-cta-h {
    font-family:'Rajdhani',sans-serif; font-size:clamp(28px,3.5vw,42px);
    font-weight:700; color:#fff; margin-bottom:10px;
}
.qa-cta-h span { color:#7DFF00; }
...
.qa-cta-btn:hover {
    background:rgba(245,166,35,.18); border-color:#7DFF00; color:#7DFF00;
    transform:translateY(-3px); box-shadow:0 8px 24px rgba(245,166,35,.2);
}
.qa-cta-btn.primary {
    background:#7DFF00; border-color:#7DFF00; color:#000;
...
<section class="qa-hero">
    ...
    <div class="qa-hero-inner qa-anim">
        ...
        <div class="qa-hero-stats qa-anim d3">
            <div class="qa-hstat">
                <span class="qa-hstat-val">{{ $heroBanner['stat1_value'] ?? $heroBanner['users'] }}</span>
                <span class="qa-hstat-lbl">{{ $heroBanner['stat1_label'] ?? 'Active Traders' }}</span>
            </div>
            <div class="qa-hstat">
                <span class="qa-hstat-val">{{ $heroBanner['stat2_value'] ?? $heroBanner['experience'] }}</span>
                <span class="qa-hstat-lbl">{{ $heroBanner['stat2_label'] ?? 'Team Experience' }}</span>
            </div>
            <div class="qa-hstat">
                <span class="qa-hstat-val">{{ $heroBanner['stat3_value'] ?? '100+' }}</span>
                <span class="qa-hstat-lbl">{{ $heroBanner['stat3_label'] ?? 'Analytics Tools' }}</span>
            </div>
            <div class="qa-hstat">
                <span class="qa-hstat-val">{{ $heroBanner['stat4_value'] ?? '50K+' }}</span>
                <span class="qa-hstat-lbl">{{ $heroBanner['stat4_label'] ?? 'Students Trained' }}</span>
            </div>
        </div>
    </div>
</section>
...
<section class="qa-ideators-sec">
    ...
    <span class="qa-sub qa-anim d2">
        The visionaries behind CityQuants — seasoned professionals who turned their passion for derivatives into India's most powerful options analytics platform.
    </span>
        ...
        <div class="qa-ideator-card qa-anim" style="animation-delay:{{ ($idx * 0.12 + 0.2) }}s">
            ...
            <div class="qa-ideator-body">
                ...
                <div class="qa-id-socials">
                    <a href="{{ $person['linkedin'] ?? '#' }}" class="qa-id-social" title="LinkedIn">
                        <i class="fab fa-linkedin-in"></i>
                    </a>
                    <a href="{{ $person['twitter'] ?? '#' }}" class="qa-id-social" title="Twitter / X">
        ...
        <div class="qa-ws-track" id="wsTrack">
            ...
            <div class="qa-ws-slide">
                ...
                <div class="qa-ws-caption">
                    ...
                    <span class="qa-ws-caption-badge">{{ $slide['tag'] ?? 'OFFICE' }}</span>
                </div>
            </div>
            @endforeach
        </div>
        ...
        <div class="qa-city-panel {{ $i===0?'on':'' }}" id="cityPanel{{ $i }}">
            <div class="qa-city-card">
                ...
                <div class="qa-city-info">
                    <div class="qa-city-flag">{{ $office['flag'] ?? '🏙️' }}</div>
...
<section class="qa-ceo-sec">
    <div class="qa-ceo-inner">
        ...
        <div class="qa-ceo-card qa-anim d1">
            ...
            <div class="qa-ceo-socials">
                @if(!empty($ceoVision['linkedin']))
                <a href="{{ $ceoVision['linkedin'] }}" class="qa-ceo-social" title="LinkedIn">
                    <i class="fab fa-linkedin-in"></i>
                </a>
                @endif
                @if(!empty($ceoVision['twitter']))
                <a href="{{ $ceoVision['twitter'] }}" class="qa-ceo-social" title="Twitter / X">
                    <i class="fab fa-x-twitter"></i>
                </a>
                @endif
                @if(!empty($ceoVision['youtube']))
                <a href="{{ $ceoVision['youtube'] }}" class="qa-ceo-social" title="YouTube">
...
<section class="qa-cta-sec">
    <h2 class="qa-cta-h qa-anim">
        Get The <span>App</span> Here!
    </h2>
