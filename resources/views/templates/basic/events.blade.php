...
<div class="qev-hero ev-anim">
    <div class="qev-hero-inner">
        <div class="qev-hero-eyebrow"><span class="qev-hero-dot"></span> Offline Events &amp; Workshops</div>
        <h1>Options Trading <span>Events</span> &amp; Workshops</h1>
        <p>Join India's top options traders, analysts & educators at CityQuants in-person events across the country.</p>
    </div>
</div>
    ...
    <div class="qev-filter-row">
        ...
        <div class="qev-filter-group">
            ...
            <select class="qev-filter-select" id="fCity" onchange="evFilter()">
                ...
                @foreach(['bangalore'=>'Bangalore','koramangala'=>'Koramangala','indiranagar'=>'Indiranagar','whitefield'=>'Whitefield','delhi'=>'Delhi','noida'=>'Noida','gurgaon'=>'Gurgaon','mumbai'=>'Mumbai','pune'=>'Pune','hyderabad'=>'Hyderabad','chennai'=>'Chennai'] as $k => $v)
                    <option value="{{ $k }}">{{ $v }}</option>
                @endforeach
            </select>
        </div>
        <div class="qev-search-wrap">
            <input class="qev-search-input" type="text" id="evSearch" placeholder="Search events..." oninput="evFilter()">
            <button class="qev-search-btn" type="button"><i class="fas fa-search"></i></button>
        </div>
    </div>
</div>
...
<div class="qev-content">
    ...
    <div class="qev-tab-panel on" id="evPanel0">
        ...
        <div class="qev-featured ev-anim">
            <div class="qev-feat-body">
                <div class="qev-feat-badge"><span class="qev-hero-dot"></span> REGISTRATIONS OPEN</div>
                <h3>{{ $featured->title }}</h3>
                @if($featured->description)<p>{{ Str::limit($featured->description, 180) }}</p>@endif
                <div class="qev-feat-meta">
                    @if($featured->formatted_date !== '—')<div class="qev-feat-meta-item"><i class="fas fa-calendar-alt"></i> {{ $featured->formatted_date }}</div>@endif
                    @if($featured->location)<div class="qev-feat-meta-item"><i class="fas fa-map-marker-alt"></i> {{ $featured->location }}</div>@endif
                    @if($featured->formatted_duration !== '—')<div class="qev-feat-meta-item"><i class="fas fa-clock"></i> {{ $featured->formatted_duration }}</div>@endif
                </div>
            ...
            <div class="qev-feat-img">
                ...
                <div class="qev-feat-price-tag">
                    ...
                    @else
                        ...
                        @if($featured->mrp > $featured->price)<div class="orig">₹{{ number_format($featured->mrp) }}</div>@endif
                        @if($featured->discount_label_auto)<div class="disc">{{ $featured->discount_label_auto }}</div>@endif
                    @endif
                </div>
            </div>
        </div>
        ...
        @if($upcomingEvents->isEmpty())
            <div class="qev-empty" style="margin-bottom:32px;"><i class="fas fa-calendar-times"></i> No upcoming events at the moment.</div>
        @else
        ...
        @if($pastEvents->isEmpty())
            <div class="qev-empty"><i class="fas fa-history"></i> No past events yet.</div>
        @else
        ...
        <div class="qev-no-results" id="evNoResults"><i class="fas fa-calendar-times"></i> No events found.</div>
    </div>
    ...
    <div class="qev-tab-panel" id="evPanel1">
        <div class="qev-section-head"><h2>Upcoming Events</h2></div>
        @if($upcomingEvents->isEmpty())
            <div class="qev-empty"><i class="fas fa-calendar-times"></i> No upcoming events.</div>
        @else
        <div class="qev-grid">
            @foreach($upcomingEvents as $ev)
            @include($activeTemplate.'partials.event-card', ['ev'=>$ev,'isPast'=>false])
...
function evTab(idx, btn) {
    ...
    document.querySelectorAll('.qev-tab-panel').forEach(function(p,i) { p.classList.toggle('on', i===idx); });
}
function evFilter() {
...
function updateCountdowns() {
    document.querySelectorAll('.qev-countdown[data-ts]').forEach(function(el) {
        ...
        if (diff <= 0) { el.querySelector('.qev-countdown-label').textContent = 'Started!'; return; }
